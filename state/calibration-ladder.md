...
| **Optuna-tuned RF on kitchen-sink** (15 trials TPE) | 0.93957 (seed 42) / 0.93961 (seed 7) | n/a | n/a | Day-20 PM Optuna probe. Best config log2/max_samples=0.7/depth=15/leaf=100/entropy. K=4+1 LR-meta Δ +0.268 bp seed=42 / +0.238 bp seed=7; cross-seed |Δ|=0.030 bp. **Across 4 independent RF runs the K=4+1 lift sits in +0.24-0.27 bp with std 0.013 bp.** Hyperparameter tuning yields zero improvement past the natural +0.25 bp ceiling — the signal is set by the meta architecture (3-D logit subspace ceiling), not by RF itself. ρ=0.9597-0.9600. |
| **Path-B C×S τ=100k on K=5 = K=4 + RF-yekenot** | 0.95405 | n/a | held (R5 hedge) | Day-20 PM Path-B refit. K=5 LR-meta global OOF 0.95402; +0.03 bp Path-B amp within fold noise. ρ vs K=4 PRIMARY 0.999917 → tie-band at LB per Rule 27 (abort threshold 0.999). PI held submission. **Path-B absorbs the +0.25 bp K=4+1 forest lift down to +0.02 bp vs PRIMARY** — confirms Day-15 friction `path-b-amp-only-fires-on-meta-arch-not-base-add`. τ=5k OOF 0.95403 (asymmetric flips 121/29 — R7 risk); τ=20k OOF 0.95405 (flips 86/23). All three τ saved as R5 hedge candidates. |
