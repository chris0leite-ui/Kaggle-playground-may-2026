...
```yaml
day: 17                           # 2026-05-07 PM. **NEW PRIMARY LB 0.95354** via `claude/optimize-model-performance-rruC2` (`d17_path_b_K23_v4_h1d_tau100000`, K=21 + p1_single_cb_v4_gpu + d17_h1d_yekenot_full, Path-B Compound×Stint τ=100k). Builds on research-branch's Day-17 PM win (`claude/read-handover-62BCt` landed K=24+h1d LB 0.95345 = +196 bp over d18 PRIMARY): h1d (yekenot RealMLP-pytabkit replication, OOF 0.95257) imported here, combined with v4 (CB recipe with yekenot FE transfer items 2/3/4 floor-cat+count+KBins + item 7 orig-aug). CB-axis arc this branch: v3 (research-recipe) OOF 0.94993 / K=21+1 +12.06 bp / LB 0.95143 → v4 OOF **0.95200** (+20.7 bp single-model, biggest CB lift on comp) / K=21+1 **+24.21 bp** (DOUBLE v3) / holdout PASS Δ−0.21 bp / G1 PASS +7.91 bp → K=23 v4+h1d Path-B τ=100k OOF 0.95415 / **LB 0.95354 (+9 bp over research-branch's 0.95345)**. Counter-evidence to research-branch audit caveat that yekenot items 2/3/4 are "NN-specific": items fired +20 bp on CatBoost too — new friction `yekenot-floor-count-kbins-fires-on-gbdt-too`. K=25 full-merge (+d16+d18) Path-B OOF 0.95428 (+1.3 bp; held, within OOF→LB noise). XGB on identical v4 FE: K=24+xgb +0.02 bp REDUNDANT to CB-v4 (`gbdt-class-redundant-on-shared-FE`). Path-B amp friction `path-b-amp-only-fires-on-meta-arch-not-base-add` re-confirmed at K=22/23/25.
...
plateau_days: 0                   # Day-17 PM ADVANCE: PRIMARY 0.95089→0.95354 in 24h (+265bp). CB-axis closed via v3→v4 yekenot transfer. Next-step priority list: (a) **K=25 full-merge** held +1.3bp may be within noise; (b) 3-seed bag of v4 (+1-3bp, 75min GPU); (c) XGB on v4 FE done — REDUNDANT to CB-v4 (gbdt-class-redundant-on-shared-FE); (d) **external Pirelli/FastF1 hard-join** (HANDOVER A4) — only structural axis with +51bp potential to top-5%; (e) RealMLP n_ens=24 (h1d ran 4); (f) cross-segmentation Path-B (Y×S, R×C) on K=23 v4+h1d.
