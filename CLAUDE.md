...
submissions_used_today: 8         # Day-17/18: K22_add_p1_feA_te 0.94933, p1_single_v1 0.94107, K2_PRIM_v2 0.94996, d16_cont_only 0.95089, d18_K23 0.95149, d17_K22_v3 0.95143, d17_K24 0.95345, d17_K23_v4_h1d 0.95354, **d18_K27_v4h1d_d16_d18_e2_f2 LB 0.95368** (NEW PRIMARY)
...
mechanism_families_explored:  # Q1 reference. Detail in audit/. Compressed 2026-05-06.
  ...
  - p1_single_cb_v3_research_recipe # Day-17 PM -- CatBoost research-backed recipe (Bernoulli + min_data_in_leaf=20 + Year/Stint cat + default CTR; no rsm on GPU). 5-fold OOF 0.94993 (+43bp over LGBM v3). K=21+1 +12.06bp (~15× any prior base-add). ρ vs PRIMARY 0.972 (most-diverse since DAE). Holdout PASS Δ-0.81bp. K=22 Path-B C×S τ=20k LB 0.95143. K=24(d16+d18+v3) Path-B held OOF 0.95247.
  - p1_single_cb_v4_yekenot_transfer # Day-17 PM -- v3 + yekenot items 2/3/4/7: floor-cat (np.floor+factorize ratios+6 cont) + count-encoding (value_counts on cats+combos numeric) + KBinsDiscretizer (200/RaceProgress + 7/LapTime) + orig-aug (aadigupta 4/5 stratified concat per fold). 5-fold OOF **0.95200** (+20.7bp over v3, biggest CB single-model lift). K=21+1 **+24.21bp** (DOUBLE v3). G1 PASS +7.91bp vs old PRIMARY. Holdout PASS Δ-0.21bp. Counter-evidence to research-branch caveat that items 2/3/4 are NN-specific — they fired +20bp on CB via CTR+split-finding interaction. New friction `yekenot-floor-count-kbins-fires-on-gbdt-too`.
  - d17_K23_v4_h1d_pathB_PRIMARY    # Day-17 PM -- K=23 = K=21 + p1_single_cb_v4 + d17_h1d_yekenot_full Path-B C×S. τ=100k OOF **0.95415** (+30bp); Path-B amp +0.12bp. SUBMITTED 2026-05-07 13:27: LB 0.95354 (PRIMARY before d18 K=27 supersession). Realised OOF→LB gap -6.1bp. K=24+xgb_v4 +0.02bp REDUNDANT (`gbdt-class-redundant-on-shared-FE`). K=25 full-merge (+d16+d18) OOF 0.95428 +1.3bp held (`pool-saturation-v4h1d-absorbs-d16d18`).
  - field_state_cross_row_aggregates  # Day-17 PM `claude/review-handover-solutions-oE78b`. Per-(R,Y,L)/(R,Y,L,Compound) aggregates over OTHER rows. fs_cum_pits AUC 0.7972 (highest single-feat on comp). Strict fold-safe audit cleared (12% collapse vs Day-17 88-100%). K=24 stack-add over K=23 v4+h1d: -0.015 bp NULL — `lr-meta-rank-lock-strong-anchor` 6th cross-confirmation. NOT submitted.
  - probe_combined_lead_lag_train_test  # Day-17 PM same branch. Train+test combined-frame lead/lag over (Race,Driver,Year). Combined-frame premium evaporates at GBDT (-0.36 bp). Rule 25 PASS via AV-AUC 0.502.
  - probe_target_structure_eda  # Day-17 PM same branch. P(target=1 | lap_from_obs_stint_end): 0.272 → 0.061 over 10 laps. 81% of multi-pos stints have last pos at observed-last-lap. Target is decay-from-end, not shifted PitStop.
  - probe_ntl_single_rule_baseline  # Day-17 PM same branch. 5 NTL reconstructions + 13 thresholded rules. Best NTL_compound_year_max AUC 0.687 < raw TyreLife 0.699. Threshold rules cap at 0.566.
  - lr_diagnostic_suite_arc_abc    # Day-18 PM 10-script LR-diagnostic battery on K=24 pool. eff_rank=2.88 of 24; E9 forward-select empirically confirmed K=10 = K=24 in OOF AUC. Promoted to skill `.claude/skills/kaggle-comp/lr-diagnostics.md` + `templates/scripts/lr_diag/`.
  - bagged_lr_with_e6_stint_cross  # Day-18 PM A2 Bagged-LR; ρ vs PRIMARY=0.71 LOWEST EVER but K=10+1 Δ +0.04 bp NULL. Friction `representation-only-diversity-meta-null-on-saturated-info-space`.
  - per_compound_lr_specialists    # Day-18 PM A4 5 LRs (one per Compound); OOF 0.87386, ρ vs PRIMARY=0.75; K=10+1 Δ +0.03 bp NULL. 6th confirmation of `rho-alone-insufficient-for-meta-utility`.
  - k10_pool_surgery_artifact      # Day-18 PM T2 forward-selected core (E9 picks 1-10) LR-meta. OOF 0.95381 vs old d18 K=24 0.95385 = -0.4 bp; ρ_test 0.99913. No-cost simplification candidate; not LB-tested. Pre-dates v4+h1d K=23 PRIMARY merge.
  - path_b_alt_segmentations_K10   # Day-18 PM T1#3 Path-B 3 alt segmentations × 3 τ on K=10. ALL 9 within sub-bp of K=10 baseline. Friction `path-b-amp-requires-large-redundant-pool-not-saturated-pool`.
  - chain_decomposition_orig_likelihood  # Day-18 PM (reverse-engineer-data-generation-Hu8EK) d18 v1 causal+gauss: K=21+1 +7.365 bp (largest single-base of arc); v2 q10 +1.43 bp (modeling-axis 5×); v3 reverse PARKED. Per-step orig-DGP log-likelihood on causal chain. LANDED in K=23 PRIMARY LB 0.95149 then K=27 LB 0.95368.
  - dgp_preimage_join_knn          # Day-18 PM same branch E2: kNN(K=10) per-Compound in orig over 7 KS-low feats; 7 aggregate features. K=21+1 +1.88 bp; solo K=23 v4+h1d+1 +0.42 bp. PASS; landed in K=27 NEW PRIMARY.
  - dgp_chain_ll_q5_pathb_cohort   # Day-18 PM same branch E5 c1: Path-B Compound×chain_LL_q5 REGRESS -1.91 to -0.04 bp. Friction `chain-ll-q5-cohort-weaker-than-compound-stint`.
  - dgp_constraint_violations      # Day-18 PM same branch F2: 10 physical constraints. y=1 rows break within-stint TyreLife consistency 19.6% more than y=0 (load-bearing class-conditional pattern). K=21+1 +1.56; solo K=23 v4+h1d+1 +0.25. PASS; landed in K=27.
  - dgp_class_cond_gmm_bayes_factor # Day-18 PM same branch F5: 2 GMMs(8) per class on 5 KS-low feats; log_ratio standalone AUC 0.873; K=21+1 +3.56; solo K=23 v4+h1d+1 +0.21. PASS.
  - dgp_cond_vector_tuple_lookup   # Day-18 PM same branch J: EB(P×C×S×R×Y).y_mean 5-way; K=21+1 +2.30; solo K=23 v4+h1d+1 +0.18. PASS.
  - dgp_ctgan_mode_id_features     # Day-18 PM same branch G/H/I: BGMM(10) per KS-low feat → mode-id as cat (G), EB(C×S×mode).y_mean (H), bias_factor synth/orig (I). K=21+1 +0.53/+0.97/+0.80 PASS individually; FULLY ABSORBED on K=23 v4+h1d (≤+0.07 bp). Friction `pool-saturation-v4h1d-absorbs-dgp-class`.
  - synth_arch_ctgan_class_replay  # Day-18 PM same branch F1 GPU replay forensics. Lowest mean KS 0.134 to CTGAN replay; non-GAN→GAN P(replay-like) jump 0.06→0.13. Host = CTGAN-class GAN with custom signature (all 4 disc AUC ≥0.988). Disc features as bases NULL (-0.11 bp). Foundational diagnostic.
  - d18_path_b_K27_v4h1d_dgp_NEW_PRIMARY # Day-18 PM same branch 🎯 NEW PRIMARY LB 0.95368 (+1.4 bp over 0.95354). K=21+v4+h1d+d16+d18+E2+F2 Path-B C×S τ=100k OOF 0.95432. PI sealed band 0-2 bp; realised +1.4 dead-center.
  - lr_bank_15plus5_rich_fe         # Day-17 PM (claude/logistic-regression-ensemble-0PNkA) -- 15-variant LR bank (kbins/ohe/poly/spline/hash) + 5 rich-FE (rozen_full, yekenot_full, dgp_rules, te_3way, mega). lr_mega OOF 0.92776 = LR ceiling. LR-bank eff_rank 2.0-2.19; combined GBDT+LR eff_rank 3.33 (+1.3 dim). Friction `lr-eff-rank-bounded-at-2-by-pipeline-not-base-class`. K=24 + mega LR sweep gate -0.89 to -36.7 bp (NULL/regress). Per-segment mega LR (Probe-5) +60.8 bp standalone (Compound×Year, MEDIUM_2023 +1081bp), but routes through cb_year-cat at meta — friction `per-segment-mega-LR-fires-only-at-LR-class-not-meta-class`. Cross-confirms sibling `lr_diagnostic_suite_arc_abc` LR-saturation finding from a different angle (FE-diversity vs eff_rank-diagnostic).
  - d18_path_b_compound_year_K24    # Day-18 (claude/logistic-regression-ensemble-0PNkA) -- Path-B (Compound×Year) τ-sweep on K=24, predicted +3 bp midpoint, ALL 4 τ NULL/regress (-2.05, -0.97, -0.16, -0.01 bp). Mechanism reconciled: K=24 has cb_year-cat → Year already routed at base level → per-segment LR-meta redundant. New friction `pathb-amp-dead-when-pool-already-routes-segmentation-variable`. NOT submitted; OOF artifacts saved for completeness/HEDGE consideration.
  - d18b_path_b_alt_axes_K24        # Day-18 (claude/logistic-regression-ensemble-0PNkA) -- 3 alt segmentation axes K=24 does not natively route: (Driver_cluster×Stint) best τ=100k Δ +0.36 bp <gate; (Race_class×TyreLife_q5) best τ=500k Δ +0.01 bp NULL; (Position_q5×Compound) best τ=500k Δ +0.00 bp NULL. 3rd cross-confirmation of `pathb-amp-dead-when-pool-already-routes-segmentation-variable`. HANDOVER A4 "alternative seg crosses" list now closed null. Path-B amp axis on K=24 pool is uniquely Compound×Stint. NOT submitted.
plateau_days: 0                   # Day-18 PM PRIMARY advance: 0.95354 → 0.95368 (+1.4 bp via combining mainline v4+h1d with DGP-class). Top-5% gap closes -5.1 → -3.7 bp. Sibling Day-18 PM (`MbLKu`) ran research-only (0 submits) confirming in-pool LR-family path exhausted. Sibling Day-17/18 PM (`0PNkA`) cross-confirmed: LR-bank diversity-via-FE bounded at eff_rank ~2.2 (lr_mega ceiling 0.92776); Path-B alt-axes (Compound×Year + Driver_cluster×Stint + Race_class×TyreLife_q5 + Position_q5×Compound) all NULL/sub-gate — Path-B amp axis on this pool uniquely Compound×Stint. Next-step priorities ordered: (a) **A1 sequence-level DGP fingerprinting** — biggest remaining DGP blind spot, mechanism-novel, predicted +1-3 bp; (b) **external Pirelli/FastF1 hard-join** (only structural axis with full-gap closure potential); (c) A2 cross-feature mode-tuple, A3 membership inference, A4 explicit-cond CTGAN replay, A5 per-Year heterogeneity (ISSUES.md 7g-7k); (d) Yao/Vehtari covariance-modelled BMA (untested meta-arch); (e) 3-seed bag of v4 + RealMLP n_ens=24.
gate_status: cleared              # d18_path_b_K27_v4h1d_d16_d18_e2_f2_tau100000 LB 0.95368 NEW PRIMARY (Day-18 PM-late); leakage-clean (orig-only chain models; K=27 pool inherits leak audits)
headroom_to_top5pct: 0.00037      # 0.95405 − 0.95368 = 3.7 bp on K=27 combined PRIMARY (closed from 5.1 bp). Reach to leader 0.95476 = -10.8 bp.
```
