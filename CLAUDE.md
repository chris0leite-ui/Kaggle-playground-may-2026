...
```yaml
day: 18                           # 2026-05-07 PM Day-18 = LR-diagnostic research session. Branch `claude/ensemble-logistic-regression-research-MbLKu` ran 3-arc LR-diagnostic expedition on K=24 pool: Arc A (E1/E2/E4/E8 pool/meta archaeology), Arc B (E5/E6/E9 DGP archaeology), Arc C (A2/A4 new-base injection). Then T2 K=10 PRIMARY artifact (-0.4 bp vs d18, ρ_test 0.99913) + T1#3 Path-B 3 alt segmentations × 3 τ on K=10 (all sub-bp). Six cross-confirmations of `rho-alone-insufficient-for-meta-utility`. New friction `path-b-amp-requires-large-redundant-pool-not-saturated-pool`. LR-diagnostic suite promoted to skill `.claude/skills/kaggle-comp/lr-diagnostics.md` + `templates/scripts/lr_diag/`. PRIMARY unchanged: `d17_K24_d18pool_h1d` LB 0.95345 (AT top-5%).
lb_best_today: 0.95435            # leader; not refreshed
our_lb_best: 0.95345              # d17_K24_d18pool_h1d (PRIMARY since Day-17 PM); AT top-5% threshold (+0.0bp gap). K=10 LR-meta candidate built (OOF 0.95381, ρ_test 0.99913 to PRIMARY) but not LB-tested.
submissions_used_today: 0         # Day-18 research-loop session (0 LB submits per Rule 1)
...
mechanism_families_explored:  # Q1 reference. Detail in audit/. Compressed 2026-05-06.
  ...
  - lr_diagnostic_suite_arc_abc    # Day-18 (2026-05-07 PM) 10-script LR-diagnostic battery on K=24. E1 entropy eff_rank=2.88 of 24 (rank-1 dominated, top-5 σ explain 91.2%); E9 forward-select empirically confirmed K=10 = K=24 in OOF AUC; E2 cb_slow-wide-bag most-redundant + most-miscalibrated AND first negative-marginal pick at K=11. Promoted to skill `.claude/skills/kaggle-comp/lr-diagnostics.md`.
  - bagged_lr_with_e6_stint_cross  # Day-18 A2 Bagged-LR; vanilla 0.85588 → rich 0.86817 (+123 bp from 9 E6-flagged Stint-cross interactions); ρ vs PRIMARY=0.71 LOWEST EVER but K=10+1 Δ +0.04 bp NULL. Origin friction `representation-only-diversity-meta-null-on-saturated-info-space`.
  - per_compound_lr_specialists    # Day-18 A4 5 LRs (one per Compound); OOF 0.87386, ρ vs PRIMARY=0.75; K=10+1 Δ +0.03 bp NULL. Cross-confirmation of A2 via structurally distinct construction. 6th confirmation of `rho-alone-insufficient-for-meta-utility`.
  - k10_pool_surgery_artifact      # Day-18 T2 forward-selected core (E9 picks 1-10) LR-meta. OOF 0.95381 vs d18 K=24 0.95385 = -0.4 bp; ρ_test 0.99913. Captures ~99.9% of K=24 signal with 60% fewer bases. No-cost simplification candidate; not LB-submitted (PI hold).
  - path_b_alt_segmentations_K10   # Day-18 T1#3 Path-B 3 alt segmentations × 3 τ on K=10. S1 Year×Compound, S2 Compound×TyreLife_q5, S3 Driver_freq_q4×Stint. ALL 9 within sub-bp of K=10 baseline; best S3 τ=20k +0.27 bp NULL. Refines friction: `path-b-amp-requires-large-redundant-pool-not-saturated-pool`.
plateau_days: 2                   # Day-17 PM advance LB 0.95345 (yekenot recipe, +19.6 bp). Day-18 0 advance (research session); LR-family in-pool path empirically exhausted across Arc A/B/C + T1#3 (6 cross-confirmations of representation-only base-add null + Path-B amp null on K=10 saturated pool). Next-session priorities (per HANDOVER Day-18 PM): T2 build K=10 PRIMARY artifact (DONE, OOF 0.95381), T1#1 non-Gaussian shrinkage (predict null), T1#2 Yao/Vehtari BMA (predict null), T3 Pirelli external data (only path with new-information mechanism; needs PI scope sign-off).
gate_status: cleared              # d17_K24_d18pool_h1d LB 0.95345 PRIMARY (Day-17 PM, +19.6 bp); K=10 LR-meta (T2) candidate built but not LB-tested
