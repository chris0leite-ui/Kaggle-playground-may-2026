...
mechanism_families_explored:  # Q1 reference. Detail in audit/. Compressed 2026-05-06.
  ...
  - lr_bank_15plus5_rich_fe         # Day-17 PM -- 15-variant LR bank (kbins/ohe/poly/spline/hash) + 5 rich-FE (rozen_full, yekenot_full, dgp_rules, te_3way, mega). lr_mega OOF 0.92776 = LR ceiling. LR-bank eff_rank 2.0-2.19; combined GBDT+LR eff_rank 3.33 (+1.3 dim). Friction `lr-eff-rank-bounded-at-2-by-pipeline-not-base-class`. K=24 + mega LR sweep gate -0.89 to -36.7 bp (NULL/regress). Per-segment mega LR (Probe-5) +60.8 bp standalone (Compound×Year, MEDIUM_2023 +1081bp), but routes through cb_year-cat at meta — friction `per-segment-mega-LR-fires-only-at-LR-class-not-meta-class`.
  - d18_path_b_compound_year_K24    # Day-18 -- Path-B (Compound×Year) τ-sweep on K=24, predicted +3 bp midpoint, ALL 4 τ NULL/regress (-2.05, -0.97, -0.16, -0.01 bp). Mechanism reconciled: K=24 has cb_year-cat → Year already routed at base level → per-segment LR-meta redundant. New friction `pathb-amp-dead-when-pool-already-routes-segmentation-variable`. NOT submitted; OOF artifacts saved for completeness/HEDGE consideration.
  - d18b_path_b_alt_axes_K24        # Day-18 -- 3 alt segmentation axes K=24 does not natively route: (Driver_cluster×Stint) best τ=100k Δ +0.36 bp <gate; (Race_class×TyreLife_q5) best τ=500k Δ +0.01 bp NULL; (Position_q5×Compound) best τ=500k Δ +0.00 bp NULL. 3rd cross-confirmation of `pathb-amp-dead-when-pool-already-routes-segmentation-variable`. HANDOVER A4 "alternative seg crosses" list now closed null. Path-B amp axis on this pool is uniquely Compound×Stint. NOT submitted.
plateau_days: 0                   # Day-17 PM ADVANCE: PRIMARY 0.95089→0.95354 in 24h (+265bp). Day-17 late + Day-18: LR-bank rich-FE ceiling 0.92776; Path-B alt-axes (Compound×Year + Driver_cluster×Stint + Race_class×TyreLife_q5 + Position_q5×Compound) ALL NULL or sub-gate. Path-B amp confirmed uniquely on Compound×Stint axis. CB-axis closed via v3→v4 yekenot transfer. Next-step priority list: (a) **K=25 full-merge** held +1.3bp may be within noise; (b) 3-seed bag of v4 (+1-3bp, 75min GPU); (c) **external Pirelli/FastF1 hard-join** (HANDOVER A4) — only structural axis with +51bp potential to top-5%; (d) RealMLP n_ens=24 (h1d ran 4); (e) per-segment NON-LR meta (e.g. tiny per-segment GBDT) — un-tested non-linearity at the meta class.
