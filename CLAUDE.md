...
mechanism_families_explored:
  ...
  - driver_cluster_path_b_cohort    # 2026-05-06 -- k-means k=4 on per-Driver stats (mean stint/soft-prop/tyre/lt-delta/pit-rate/n-laps) → cluster cohort; Compound×cluster (5×4=20 segs) Path B; -0.4 to -0.9 bp NULL across τ. Cohort axis exhausted.
  - alpha_calibrated_tau_resweep    # 2026-05-06 -- Path B α computed with full-train counts at OOF (matches test); τ=20k UNCHANGED (ρ=1.0, Δ -0.02 bp). Smaller τ regress (amplifies per-segment LR noise). PRIMARY's τ=20k empirically optimal.
  - id_order_synth_artifact         # 2026-05-06 -- LapNumber_mod_10 marginal span 566 bp, id_mod_1000 568 bp; test ids 0/188165 in train range. Marginal span DOES NOT translate to predictive lift when GBDT has feature interactions; finding logged but lever dead.
  - target_reformulation_invlaps    # 2026-05-06 -- LGBM regression on 1/(1+laps_until_pit), target-derived (NOT meta-derivative). Std OOF 0.94053 (-103 bp), ρ=0.924. K=21+1 +1.899 bp OOF (largest non-meta-derivative single-add lift ever measured). HELD candidate.
  - target_reformulation_stintprog  # 2026-05-06 -- LGBM regression on TyreLife/max(stint), std 0.64851 (-3023 bp), ρ=0.252 (most-diverse single base ever); K=21+1 alone NULL but +0.10 bp jointly with inv_laps. HELD as orthogonal-extreme calibration data.
  - multi_target_nn_pit_aux_invlaps # 2026-05-06 -- shared-trunk NN with pit_next_lap (BCE) + inv_laps_until_pit (MSE 0.3 weight) heads; std 0.92295, K=21+1 +0.086 bp NULL. Aux head ineffective at meta gate.
  - path_b_K22_invlaps_compound_stint # 2026-05-06 -- Path B Compound×Stint hier-meta on K=22 = K=21 + inv_laps_until_pit; OOF 0.95110 (+2.75 bp vs PRIMARY) at τ=20k, ρ=0.99753. **LARGEST OOF ADVANCE OF SESSION**, target-derived (orthogonal-signal criterion satisfied). Held pending PI submission decision; predicted LB +1.25 to +32 bp depending on amp factor.
...
| **d13e_compound_stint_tau20000** | **0.95083** | n/a | **0.95049** | **NEW PRIMARY**; +8bp LB over Path B Stint; 8× OOF upside (+1.00bp OOF); ρ=0.9958 vs d9f; 24 seg cross |
