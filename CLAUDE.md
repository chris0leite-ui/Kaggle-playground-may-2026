...
```yaml
day: 17                           # 2026-05-07 PM. **NEW PRIMARY LB 0.95354** via `claude/optimize-model-performance-rruC2` (`d17_path_b_K23_v4_h1d_tau100000`, K=21 + p1_single_cb_v4_gpu + d17_h1d_yekenot_full, Path-B Compound×Stint τ=100k). CB-axis arc Day-17 PM: v3 (research-recipe) OOF 0.94993 / K=21+1 +12.06 bp / LB 0.95143 → v4 (yekenot transfer items 2/3/4 floor-cat+count+KBins + item 7 orig-aug) OOF **0.95200** (+20.7 bp single-model, biggest CB lift on comp) / K=21+1 **+24.21 bp** (DOUBLE v3) / holdout PASS Δ−0.21 bp / standalone G1 PASS +7.91 bp vs prior PRIMARY → K=23 v4+h1d Path-B τ=100k OOF 0.95415 / **LB 0.95354 NEW PRIMARY (+265 bp over yesterday's PRIMARY 0.95089, +9 bp over research-branch K=24+h1d 0.95345)**. Research-branch `claude/read-handover-62BCt` landed K=24+h1d at LB 0.95345 earlier today; h1d (yekenot RealMLP-pytabkit replication, OOF 0.95257) artifacts imported here. Path-B amp friction `path-b-amp-only-fires-on-meta-arch-not-base-add` re-confirmed (+0.12 bp over flat K=23 LR-meta).
lb_best_today: 0.95476            # MILANFX (refreshed via leaderboard download Day-17 PM)
our_lb_best: 0.95354              # **rank #98 of 893 = top 11%**; tied with [QWERTY] Roman Rozen at 0.95354. Top-5% boundary moved to 0.95405 (rank 44). Gap to top-5% −51 bp.
submissions_used_today: 7         # Day-17: K22_add_p1_feA_te 0.94933, p1_single_v1 0.94107, K2_PRIM_v2 0.94996, d16_cont_only 0.95089, d18_K23 0.95149, d17_K22_v3 0.95143, d17_K23_v4_h1d 0.95354 (NEW PRIMARY)
...
mechanism_families_explored:
  ...
  - target_reformulation_invlaps    # 2026-05-06 -- LGBM regression on 1/(1+laps_until_pit), target-derived (NOT meta-derivative). Std OOF 0.94053 (-103 bp), ρ=0.924. K=21+1 +1.899 bp OOF originally — **but STRICT-OOF audit collapsed to +0.234 bp (88% collapse)**. Original win was target-construction-layer-leakage (compute_targets() per-group aggregation used va-row labels). Genuine signal ≤+0.3 bp. Held submission INVALIDATED.
  - target_reformulation_stintprog  # 2026-05-06 -- LGBM regression on TyreLife/max(stint), std 0.64851, ρ=0.252; same target-construction-layer-leakage pattern; legit signal not measured but expected ≈0.
  - target_reformulation_pit_horizon # 2026-05-06 -- LGBM regression on 4-class horizon bucket. K=21+1 +3.191 bp originally → strict-OOF +0.302 bp (90% collapse). Same leakage as inv_laps.
  - target_reformulation_reverse_cum # 2026-05-06 -- LGBM regression on # remaining pits per Driver-Race-Year. K=21+1 +4.867 bp originally (BIGGEST single-add of session) → strict-OOF -0.005 bp (100% collapse). Same leakage. Genuine signal essentially zero. Audit at audit/2026-05-06-target-reform-leakage-audit.md.
  - target_construction_layer_leakage_audit # 2026-05-06 -- generalised friction `target-construction-layer-leakage`: compute_targets() per (Driver, Race, Year) group using ALL train labels leaks val-row labels into tr-row targets via group-level total_pits + cumsum. LightGBM training is technically (X[tr], target[tr]) but target[tr] sees va-row labels indirectly. Same failure mode as d12_lr_meta. ALL THREE target-reform single-adds tested (reverse_cum, pit_horizon, inv_laps) confirmed inflated 88-100%. Joint K=21+3 strict +0.275 bp (vs +7.667 original, 96% collapse). All K=23/K=25 Path B variants built on these targets are similarly inflated; **DO NOT submit** any of: `path_b_K22_invlaps_*`, `path_b_K23_dae_invlaps_*`, `path_b_K25_megapool_*`.
  - multi_target_nn_pit_aux_invlaps # 2026-05-06 -- shared-trunk NN with pit_next_lap (BCE) + inv_laps_until_pit (MSE 0.3) heads; std 0.92295, K=21+1 +0.086 bp NULL.
  - path_b_K22_invlaps_compound_stint # 2026-05-06 -- Path B Compound×Stint hier-meta on K=22 = K=21 + inv_laps_until_pit; OOF 0.95110 (+2.75 bp vs PRIMARY) at τ=20k originally — **BUT subsequently FALSIFIED via strict-OOF audit** (inv_laps was the leaky 22nd base). Held submission INVALIDATED. Path-B-amp on top of leaky-base is leaky too.
  - path_b_multilevel_4tier        # 2026-05-06 -- 4-tier hier-meta (Compound×Stint×Year / Compound×Stint / Compound / global) on K=22 = K=21+DAE; 5 (τ_0,τ_1,τ_2) configs; ALL NULL (-0.16 to -0.79 bp vs PRIMARY). Multi-level hierarchy with sparse L0 cells doesn't fire Path-B amp. T4a meta-arch redesign hypothesis FALSIFIED.
  - target_reform_strict_oof_audit  # 2026-05-06 -- strict-OOF re-runs (per-fold target construction using only y[tr]). 3-of-3 candidates produce ≤+0.3 bp K=21+1; joint ≤+0.3 bp; 88-100% of the original "wins" were leakage. Confirms session's target-reformulation thesis was a mirage; PRIMARY remains LB 0.95059 (DAE-only K=22, no target-derivation).
  ...
  - p1_single_cb_v3_research_recipe # Day-17 PM -- CatBoost research-backed recipe (Bernoulli + min_data_in_leaf=20 + Year/Stint cat + default CTR + one_hot_max_size=10, no rsm on GPU). 5-fold OOF 0.94993 (+43 bp over LGBM v3 honest 0.94563). K=21+1 LR-meta lift +12.06 bp (~15× any prior base-add). ρ vs PRIMARY 0.972 (most-diverse since DAE 0.948). Holdout (Rule 24) PASS Δ-0.81 bp. K=22 Path-B C×S τ=20k OOF 0.95200 → LB 0.95143. K=24 (d16+d18+v3) Path-B held at OOF 0.95247.
  - p1_single_cb_v4_yekenot_transfer # Day-17 PM -- v3 + yekenot FE items: floor-cat (np.floor+factorize for ratios+6 cont), count-encoding (value_counts on cats+combos as numeric), KBinsDiscretizer (n_bins=200 RaceProgress + 7 LapTime), orig-augmentation (aadigupta1601 4/5 stratified concat per fold). Standalone OOF **0.95200** (+20.7 bp over v3, biggest CB single-model lift on comp). K=21+1 LR-meta lift **+24.21 bp** (DOUBLE v3). G1 PASS (+7.91 bp vs old PRIMARY 0.95121 standalone). ρ standalone 0.971 / meta-projected 0.991. G3 flip ratio 0.967, 1271 total flips. Holdout (Rule 24) PASS Δ-0.21 bp. K=22 Path-B C×S τ=20k OOF 0.95319 (+19.79 bp). Confirms audit caveat WRONG: items 2-4 fired on CatBoost despite being labelled "NN-specific" — yekenot recipe transfers to GBDT via CTR + split-finding interaction with explicit floor/count/KBins inputs.
  - d17_K23_v4_h1d_pathB_NEW_PRIMARY # Day-17 PM -- K=23 = K=21 + p1_single_cb_v4_gpu + d17_h1d_yekenot_full Path-B Compound×Stint hier-meta. Best τ=100k OOF **0.95415** (+29.4 bp over old PRIMARY OOF 0.95121, +0.30 bp over research-branch K=24+h1d 0.95385). Path-B amp +0.12 bp (friction `path-b-amp-only-fires-on-meta-arch-not-base-add` re-confirmed; lift in base-add layer). G3 flip ratio 0.722, 952 total flips. **SUBMITTED 2026-05-07 13:27: LB 0.95354** (+265 bp over yesterday's PRIMARY 0.95089, +9 bp over research-branch K=24+h1d at LB 0.95345). NEW PRIMARY. Realised OOF→LB gap −6.1 bp. **Rank #98 of 893; gap to top-5% (0.95405) −51 bp.**
plateau_days: 0                   # Day-17 PM advance: PRIMARY moved from LB 0.95089 (yesterday) → 0.95354 (today). +265 bp swing in 24 hr. Top-5% boundary moved up 60 bp (was 0.95345 stale; now 0.95405). Next-step priority: (a) **K=25 = K=21 + v4 + h1d + d16 + d18** (full merge of today's wins; +3-8 bp predicted, ~10 min); (b) 3-seed bag of v4 (+1-3 bp, ~75 min Kaggle GPU); (c) XGB with v4-recipe FE (+5-15 bp, ~30 min Kaggle GPU; new model class on same FE); (d) **external Pirelli/FastF1 hard-join** (HANDOVER A4, untested, only structural axis with +51 bp potential to top-5%); (e) full RealMLP n_ens=24 (h1d ran 4); (f) cross-segmentation Path-B (Year×Stint, Race×Compound) on K=23 v4+h1d pool.
gate_status: cleared              # d17_path_b_K23_v4_h1d_tau100000 LB 0.95354 PRIMARY (Day-17 PM)
headroom_to_top5pct: 0.00051      # 0.95405 − 0.95354 = 51 bp (Day-17 PM; was 28.6 bp on stale 0.95345 threshold; top-5% boundary moved up 60 bp via competitor advances; we cleared the OLD threshold but the bar moved)
```
