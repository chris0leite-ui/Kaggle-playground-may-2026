...
```yaml
day: 18                           # 2026-05-07 PM. **PRIMARY LB 0.95354** (sibling `claude/optimize-model-performance-rruC2`: `d17_path_b_K23_v4_h1d_tau100000`, K=21 + p1_single_cb_v4_gpu + d17_h1d_yekenot_full, Path-B C×S τ=100k). Builds on Day-17 PM `claude/read-handover-62BCt` K=24+h1d LB 0.95345 + h1d (yekenot RealMLP-pytabkit replication) + v4 (CB recipe with yekenot FE items 2/3/4 floor-cat+count+KBins + item 7 orig-aug). v4 5-fold OOF 0.95200 (+20.7 bp single-model). K=23 v4+h1d Path-B τ=100k LB 0.95354. New friction `yekenot-floor-count-kbins-fires-on-gbdt-too` (items fired +20 bp on CB). K=25 full-merge held +1.3 bp. XGB on v4 FE: REDUNDANT (`gbdt-class-redundant-on-shared-FE`). **Day-18 PM** branch `claude/ensemble-logistic-regression-research-MbLKu` ran 3-arc LR-diagnostic expedition (Arc A/B/C: 10 scripts) + 2 Tier-1 meta-arch tests (T2 K=10 PRIMARY artifact OOF 0.95381 −0.4 bp; T1#3 Path-B 3 alt segmentations × 3 τ ALL sub-bp NULL). Six cross-confirmations of `rho-alone-insufficient-for-meta-utility`. New friction `path-b-amp-requires-large-redundant-pool-not-saturated-pool`. LR-diagnostic suite promoted to skill `.claude/skills/kaggle-comp/lr-diagnostics.md` + `templates/scripts/lr_diag/` (Day-1 toolkit for next Featured comp).
lb_best_today: 0.95476            # MILANFX (refreshed Day-17 PM)
our_lb_best: 0.95354              # **rank #98 of 893 = top 11%**; tied with [QWERTY] Roman Rozen. Top-5% boundary moved to 0.95405 (rank 44). Gap to top-5% −51 bp. K=10 LR-meta candidate (Day-18 T2) OOF 0.95381 ρ_test 0.99913 to old PRIMARY 0.95345 (built; not LB-tested).
submissions_used_today: 7         # Day-17 (Day-18 PM 0 submits research-only): K22_add_p1_feA_te 0.94933, p1_single_v1 0.94107, K2_PRIM_v2 0.94996, d16_cont_only 0.95089, d18_K23 0.95149, d17_K22_v3 0.95143, d17_K23_v4_h1d 0.95354 (NEW PRIMARY)
submissions_used_total: 38
...
mechanism_families_explored:  # Q1 reference. Detail in audit/. Compressed 2026-05-06.
  ...
  - d17_K23_v4_h1d_pathB_NEW_PRIMARY # Day-17 PM -- K=23 = K=21 + p1_single_cb_v4 + d17_h1d_yekenot_full Path-B C×S. τ=100k OOF **0.95415** (+30bp); Path-B amp +0.12bp (friction `path-b-amp-only-fires-on-meta-arch-not-base-add` re-confirmed). G3 flip 0.722, 952 flips. **SUBMITTED 2026-05-07 13:27: LB 0.95354 NEW PRIMARY** (+9bp over research K=24+h1d 0.95345). Realised OOF→LB gap -6.1bp. K=24+xgb_v4 +0.02bp REDUNDANT (`gbdt-class-redundant-on-shared-FE`). K=25 full-merge (+d16+d18) OOF 0.95428 +1.3bp held (`pool-saturation-v4h1d-absorbs-d16d18`).
  - field_state_cross_row_aggregates  # Day-17 PM `claude/review-handover-solutions-oE78b`. Per-(R,Y,L) and per-(R,Y,L,Compound) aggregates over OTHER rows: n_drivers_pitting, fs_cum_pits (Σ PitStop in race so far), pit_rate_now, mean/std TyreLife. PitStop is feature col not label (single-feat AUC 0.521 ≈ chance per U2). Standalone single-LGBM raw 14 + ~30 fs feats: full-train OOF 0.94230 (+15.58 bp), strict per-fold OOF 0.94211 (+13.73 bp). Top single-feat fs_cum_pits AUC 0.7972 (highest single-feat on comp). Strict fold-safe audit cleared (12% collapse vs Day-17 88-100%). K=24 stack-add over K=23 v4+h1d: marginal -0.015 bp NULL — `lr-meta-rank-lock-strong-anchor` 6th cross-confirmation. NOT submitted.
  - probe_combined_lead_lag_train_test  # Day-17 PM same branch. Train+test combined-frame lead/lag over (Race,Driver,Year). Single-feat L1: lead_LapNumber_diff +29bp combined-vs-train-only. LGBM raw+combined 0.94096 (+2.18 bp) vs raw+train-only 0.94099 (+2.55 bp); combined-frame premium -0.36 bp. Combined-frame premium evaporates at GBDT — model extracts sequence from raw interactions implicitly. Rule 25 PASS via AV-AUC 0.502.
  - probe_target_structure_eda  # Day-17 PM same branch. EDA on PitNextLap. P(target=1 | lap_from_obs_stint_end): 0.272 → 0.061 over 10 laps. 81% of multi-pos stints have last pos at observed-last-lap; 65% contiguous. Only 25% of positives align with any next-row state change. Stint sz=4 has pos_max=4. Target is decay-from-end, not shifted PitStop, not stint-boundary deterministic.
  - probe_ntl_single_rule_baseline  # Day-17 PM same branch. 5 NTL reconstructions + 13 thresholded rules. Best NTL_compound_year_max AUC 0.687 < raw TyreLife alone 0.699. Threshold rules cap at 0.566. Host's brief.md "trivial" refers to unmasked original column (5.5% hard-join only).
  - lr_diagnostic_suite_arc_abc    # Day-18 PM 10-script LR-diagnostic battery on K=24 pool. E1 entropy eff_rank=2.88 of 24 (rank-1 dominated, top-5 σ explain 91.2%); E9 forward-select empirically confirmed K=10 = K=24 in OOF AUC; E2 cb_slow-wide-bag most-redundant + most-miscalibrated AND first negative-marginal pick at K=11. Promoted to skill `.claude/skills/kaggle-comp/lr-diagnostics.md` + `templates/scripts/lr_diag/`.
  - bagged_lr_with_e6_stint_cross  # Day-18 PM A2 Bagged-LR; vanilla 0.85588 → rich 0.86817 (+123 bp from 9 E6-flagged Stint-cross interactions); ρ vs PRIMARY=0.71 LOWEST EVER but K=10+1 Δ +0.04 bp NULL. Origin friction `representation-only-diversity-meta-null-on-saturated-info-space`.
  - per_compound_lr_specialists    # Day-18 PM A4 5 LRs (one per Compound); OOF 0.87386, ρ vs PRIMARY=0.75; K=10+1 Δ +0.03 bp NULL. Cross-confirmation of A2 via structurally distinct construction. 6th confirmation of `rho-alone-insufficient-for-meta-utility`.
  - k10_pool_surgery_artifact      # Day-18 PM T2 forward-selected core (E9 picks 1-10) LR-meta. OOF 0.95381 vs old d18 K=24 0.95385 = -0.4 bp; ρ_test 0.99913. Captures ~99.9% of K=24 signal with 60% fewer bases. No-cost simplification candidate; not LB-tested. NOTE: pre-dates v4+h1d K=23 LB 0.95354 PRIMARY merge — needs re-baselining vs current PRIMARY pool.
  - path_b_alt_segmentations_K10   # Day-18 PM T1#3 Path-B 3 alt segmentations × 3 τ on K=10 (S1 Year×Compound, S2 Compound×TyreLife_q5, S3 Driver_freq_q4×Stint). ALL 9 within sub-bp of K=10 baseline; best S3 τ=20k +0.27 bp NULL. Refines friction: `path-b-amp-requires-large-redundant-pool-not-saturated-pool` (Path-B is a redundancy-re-allocation mechanism, not a new-information mechanism).
plateau_days: 0                   # Day-17 PM ADVANCE: PRIMARY 0.95089→0.95354 in 24h (+265bp). Day-18 PM research-only (0 LB submits, 12 diagnostic experiments + 2 meta-arch tests; in-pool LR-family path empirically exhausted across Arc A/B/C + T1#3). Next-step priorities: (a) K=25 full-merge held +1.3bp may be within noise; (b) 3-seed bag of v4 (+1-3bp, 75min GPU); (c) **external Pirelli/FastF1 hard-join** (HANDOVER A4) — only structural axis with +51bp potential to top-5%; (d) RealMLP n_ens=24 (h1d ran 4); (e) re-baseline T2 K=10 candidate against v4+h1d PRIMARY pool.
gate_status: cleared              # d17_path_b_K23_v4_h1d_tau100000 LB 0.95354 NEW PRIMARY (Day-17 PM, leakage-clean per Rule 24 audit on v4 recipe Δ-0.21bp)
headroom_to_top5pct: 0.00051      # 0.95405 − 0.95354 = 51bp (Day-17 PM; top-5% boundary moved 60bp from stale 0.95345 to 0.95405; we cleared OLD threshold but bar moved). Rank #98 of 893 (top 11%); reach to leader 0.95476 = -122bp.
```
