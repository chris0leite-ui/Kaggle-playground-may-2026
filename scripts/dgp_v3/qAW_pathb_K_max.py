"""qAW — Path-B C×S τ-sweep on the maximal pool.

K=N where N = K=4 + every available decoded extra. Tests if Path-B
amplifies the +1.066 bp K=12 plain LR-meta result further.
"""
...
def main():
    ...
    EXTRAS = [
        ...
        ("qAJ",  "dgp_v3_qAJ_stint_orig_drv_oof.npy", "dgp_v3_qAJ_stint_orig_drv_test.npy"),
    ...
    for nm, oof_f, test_f in BASES + EXTRAS:
        base_oofs.append(_pos(ART / oof_f))
        base_tests.append(_pos(ART / test_f))
        names.append(nm)
    print(f"K={len(names)} pool: {names}")
    ...
    print(f"\nFolding hier-meta on K={len(names)}...")
    ...
    out = {"PRIMARY_oof_auc": primary_oof_auc, "K_max_path_b_cs": {}, "names": names}
    ...
    for tau in taus:
        ...
        out["K_max_path_b_cs"][f"tau_{tau}"] = {
        ...
        np.save(ART / f"dgp_v3_qAW_Kmax_pathb_tau{tau}_oof.npy", oofs[tau])
        np.save(ART / f"dgp_v3_qAW_Kmax_pathb_tau{tau}_test.npy", test_preds[tau])

        sub = pd.DataFrame({"id": test["id"].values, TARGET: test_preds[tau]})
        sub_path = Path("submissions") / f"submission_Kmax_all_decoded_pathb_cs_tau{tau}.csv"
        sub_path.parent.mkdir(exist_ok=True)
        sub.to_csv(sub_path, index=False)

    fp = ART / "dgp_v3_qAW_Kmax_pathb.json"
