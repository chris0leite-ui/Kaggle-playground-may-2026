"""qAN — Path-B C×S τ-sweep on K=7 = K=4 + qAK + qAA + qAF.

Best K=7 combo from qAM: +0.983 bp at plain LR-meta OOF.
Test whether Path-B Compound × Stint shrinkage amplifies this into
LB territory.
"""
...
def main():
    ...
    print(f"PRIMARY LB: 0.95351")

    out = {"PRIMARY_oof_auc": primary_oof_auc, "K7_path_b_cs": {}, "names": names}
    ...
    for tau in taus:
        ...
        np.save(ART / f"dgp_v3_qAN_K7_pathb_tau{tau}_oof.npy", oofs[tau])
        np.save(ART / f"dgp_v3_qAN_K7_pathb_tau{tau}_test.npy", test_preds[tau])

        # Submission file
        sub = pd.DataFrame({"id": test["id"].values, TARGET: test_preds[tau]})
        sub_path = Path("submissions") / f"submission_K7_qAK_qAA_qAF_pathb_cs_tau{tau}.csv"
        sub_path.parent.mkdir(exist_ok=True)
        sub.to_csv(sub_path, index=False)

    fp = ART / "dgp_v3_qAN_K7_pathb.json"
