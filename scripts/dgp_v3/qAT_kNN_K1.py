"""qAT — orig kNN K=1 strictest per-row identity (3 features only).

qAK with K=3 escapes rank-lock at +0.717 bp. qAT pushes the slim-base
hypothesis further: K=1 nearest orig neighbour, 3 features (label,
distance, level_used). If slim-er = better, this should match or
exceed qAK.
"""
...
def main():
    ...
    print("Build test K=1...")
    te_l, te_d, te_lv = kNN_K1(test)
    print(f"  train levels: {pd.Series(tr_lv).value_counts().sort_index().to_dict()}", flush=True)
    print(f"  test levels:  {pd.Series(te_lv).value_counts().sort_index().to_dict()}", flush=True)
    ...
    print(f"\nqAT standalone OOF AUC = {auc:.5f}", flush=True)
    ...
    np.save(ART / "dgp_v3_qAT_K1_oof.npy", oof)
    np.save(ART / "dgp_v3_qAT_K1_test.npy", test_pred)
    out = {"oof_auc": auc, "fold_aucs": fold_aucs, "rho_test": rho_test,
           "rho_oof": rho_oof, "k4plus1_lift_bp": delta}
    fp = ART / "dgp_v3_qAT_K1.json"
