"""scripts/probe_path_b_K22_d12meta.py — Path B over K=22 amplification probe.
...
K22_EXTRA = "d12_lr_meta"  # the +1.348 bp ablation winner
...
def main():
    ...
    for tau in TAUS:
        ...
        np.save(ART / f"oof_path_b_K22_d12meta_compound_stint_tau{tau}_strat.npy",
                np.column_stack([1 - oof, oof]))
        np.save(ART / f"test_path_b_K22_d12meta_compound_stint_tau{tau}_strat.npy",
                np.column_stack([1 - tp, tp]))
        sub = sample_sub.copy(); sub[TARGET] = tp
        sub.to_csv(f"submissions/submission_path_b_K22_d12meta_tau{tau}.csv",
                   index=False)
    ...
    out = ART / "probe_path_b_K22_d12meta.json"
