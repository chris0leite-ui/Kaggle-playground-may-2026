"""scripts/probe_r15_cb_tweedie.py — R15 Phase 3: CB Tweedie on LapsUntilPit.

BASE distributional-loss alternative. Native CatBoost
`Tweedie:variance_power=1.5` — power-law deviance, suited for
positive-skewed count targets. Different gradient from RMSE
(used by cb_horizon on the same target's log transform).

Target: raw LapsUntilPit count (integer, no log transform). Strict
per-fold derivation per cb_horizon convention.

Output: predicted positive count → rank-normalized to (eps, 1-eps)
for Path-B K=N+ stack-add.

Usage:
  python scripts/probe_r15_cb_tweedie.py [--smoke] [--max-rounds 3000]
"""
...
def cb_tweedie_params(max_iters: int, seed: int, depth: int = 8) -> dict:
    return dict(
...
def main() -> None:
    ...
    t0 = time.time()
    print("== R15 Phase 3: cb_tweedie (CB Tweedie on raw LapsUntilPit count) ==",
          flush=True)
    ...
    for fold, (ti, vi) in enumerate(fold_list[:n_eff_folds], 1):
        ...
        # Invert log1p to get raw counts (Tweedie expects non-negative)
        t_ti = np.expm1(t_ti_log).astype(np.float32)
        t_va = np.expm1(t_va_log).astype(np.float32)
        print(f"    target_ti: mean {t_ti.mean():.3f}  std {t_ti.std():.3f}  "
              f"target_va: mean {t_va.mean():.3f}", flush=True)

        params = cb_tweedie_params(args.max_rounds, SEED, depth=args.depth)
        ...
        # Use -pred as rank signal (so high score = soon pit)
        score_va = -pred_va
        score_te = -pred_te
    ...
    # ρ vs R15 PRIMARY OOF (current PRIMARY = K=17 + Path-B DCS)
    ...
    # ρ vs cb_horizon (same target, different loss)
    cbh = np.load(ART / "oof_R12_cb_horizon_strat.npy")
    rho_vs_cbh, _ = spearmanr(oof_pred, cbh)
    print(f"  ρ_OOF vs cb_horizon (same target, RMSE loss): {rho_vs_cbh:.6f}",
          flush=True)
    ...
    np.save(ART / "oof_R15_cb_tweedie_strat.npy",
            oof_uniform[order_back_train].astype(np.float32))
    np.save(ART / "test_R15_cb_tweedie_strat.npy",
            test_uniform[order_back_test].astype(np.float32))
    print(f"  Saved oof_R15_cb_tweedie_strat.npy + test_..._strat.npy",
          flush=True)

    summary = dict(
        round="R15_Phase3_cb_tweedie",
        oof_auc=auc_full,
    ...
    out_json = Path("audit/2026-05-19-r15-cb_tweedie.json")
