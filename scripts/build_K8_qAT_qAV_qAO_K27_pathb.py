"""Build K=8 = K=4 forward-greedy + qAT + qAV + qAO + K=27 super-base
under per-segment shrinkage (Path-B C x Stint) at tau=100k.

Today's first submission candidate. The K=4 pool reached LB 0.95351;
adding the K=27 super-base reached LB 0.95368; adding three slim
nearest-neighbour bases on top is the new combination.

Outputs:
  artifacts/K8_qAT_qAV_qAO_K27_pathb_tau100000_oof.npy
  artifacts/K8_qAT_qAV_qAO_K27_pathb_tau100000_test.npy
  artifacts/K8_qAT_qAV_qAO_K27_pathb_tau100000.json
  artifacts/submission_K8_qAT_qAV_qAO_K27_pathb_tau100000.csv
"""
...
def main() -> None:
    ...
    # Plain LR-meta sanity (no per-segment shrinkage) — what the K=8 stack scores
    # without the segment-aware refinement layer.
    plain_oof = lr_meta_oof(expand(np.column_stack(all_oof)), y)
    plain_auc = float(roc_auc_score(y, plain_oof))
    print(f"\nK=8 plain LR-meta OOF AUC: {plain_auc:.5f}", flush=True)

    # K=4 anchor for delta-bp readout
    K4_oof_anchor = lr_meta_oof(expand(np.column_stack(all_oof[:4])), y)
    K4_auc = float(roc_auc_score(y, K4_oof_anchor))
    print(f"K=4 anchor OOF AUC:        {K4_auc:.5f}  (delta = {(plain_auc - K4_auc) * 1e4:+.3f} bp)", flush=True)

    # K=4 + Path-B reference (on disk)
    ...
    print(f"\n--- Running Path-B C x Stint tau={TAU} on K=8 ---", flush=True)
    oof, test_pred = run_pathb(all_oof, all_test, train, test, y, tau=TAU)
    auc = float(roc_auc_score(y, oof))
    print(f"\nK=8 + Path-B OOF AUC:      {auc:.5f}", flush=True)
    ...
    # Pre-submit diagnostics: rho vs K=27 + Path-B reference (current best on disk)
    ...
    print(f"rho_test vs K=27 + Path-B: {rho_test:.6f}  (Rule 27 threshold 0.999)", flush=True)
    verdict = "ABORT_TIE" if rho_test > 0.999 else "OK_TO_SUBMIT"
    print(f"Verdict: {verdict}", flush=True)

    # Save artifacts
    np.save(ART / f"K8_qAT_qAV_qAO_K27_pathb_tau{TAU}_oof.npy", oof)
    np.save(ART / f"K8_qAT_qAV_qAO_K27_pathb_tau{TAU}_test.npy", test_pred)

    # CSV submission
    sub = pd.DataFrame({"id": test["id"], "PitNextLap": test_pred})
    csv_path = ART / f"submission_K8_qAT_qAV_qAO_K27_pathb_tau{TAU}.csv"
    sub.to_csv(csv_path, index=False)
    print(f"\nWrote {csv_path.name}  ({len(sub)} rows)", flush=True)

    # Summary JSON for the audit trail
    summary = {
        "bases": names,
        "tau": TAU,
        "K4_anchor_plain_oof": K4_auc,
        "K8_plain_oof": plain_auc,
    ...
    (ART / f"K8_qAT_qAV_qAO_K27_pathb_tau{TAU}.json").write_text(
