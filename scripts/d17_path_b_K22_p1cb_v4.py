"""d17 Path-B C×S on K=22 = K=21 + p1_single_cb_v4_gpu.
...
Outputs (all under scripts/artifacts/):
  oof_d17_path_b_K22_p1cb_v4_tau{tau}_strat.npy + test_..._strat.npy
  d17_path_b_K22_p1cb_v4_results.json
  submissions/submission_d17_path_b_K22_p1cb_v4_tau{tau}.csv
"""
...
# d17 22nd base
D17_NEW = [("p1_single_cb", "p1_single_cb_v4_gpu")]
...
def main():
    ...
    for tau in taus:
        ...
        print(f"    flips: +→− {flips_to_neg}, −→+ {flips_to_pos}, "
              f"ratio {ratio:.3f}")
        np.save(ART / f"oof_d17_path_b_K22_p1cb_v4_tau{tau}_strat.npy",
                np.column_stack([1 - oof, oof]))
        np.save(ART / f"test_d17_path_b_K22_p1cb_v4_tau{tau}_strat.npy",
                np.column_stack([1 - tp, tp]))
        sub = sample_sub.copy(); sub[TARGET] = tp
        Path("submissions").mkdir(exist_ok=True)
        sub.to_csv(f"submissions/submission_d17_path_b_K22_p1cb_v4_tau{tau}.csv",
                   index=False)
    ...
    (ART / "d17_path_b_K22_p1cb_v4_results.json").write_text(
        json.dumps(final, indent=2))
    print(f"\n→ scripts/artifacts/d17_path_b_K22_p1cb_v4_results.json  "
