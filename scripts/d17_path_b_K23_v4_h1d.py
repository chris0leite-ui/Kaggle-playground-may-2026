"""d17 Path-B C×S on K=23 = K=21 + p1_single_cb_v4_gpu.
...
Outputs (all under scripts/artifacts/):
  oof_d17_path_b_K23_v4_h1d_tau{tau}_strat.npy + test_..._strat.npy
  d17_path_b_K23_v4_h1d_results.json
  submissions/submission_d17_path_b_K23_v4_h1d_tau{tau}.csv
"""
...
TARGET = "PitNextLap"
SEED, N_FOLDS = 42, 5
PRIMARY_LB_OOF = 0.95121  # d16 cont_only K=23 Path-B (current PRIMARY)
MIN_ROWS = 1000
MAX_ITER = 500
...
# d17 22nd base
D17_NEW = [("p1_single_cb_v4", "p1_single_cb_v4_gpu"), ("h1d_yekenot", "d17_h1d_yekenot_full")]
...
def main():
    ...
    print(f"  Global K=23 LR meta OOF: {auc_global:.5f}")

    # Hier-meta sweep
    taus = [5000, 20000, 100000, 500000]
    oofs = {tau: np.zeros(len(y)) for tau in taus}

    print("\n--- Compound×Stint hier-meta on K=23 ---")
    for fold, (tr, va) in enumerate(splits):
        t_fold = time.time()
    ...
    print("\n=== d17 Path-B K=23 sweep (vs PRIMARY d16 cont_only K=23 OOF 0.95121) ===")
    ...
    for tau in taus:
        ...
        print(f"    Strat OOF: {auc:.5f}  Δ vs PRIMARY OOF: {d_oof_primary:+.2f}bp  "
              f"Δ vs K=23 global: {d_oof_global:+.2f}bp")
        print(f"    ρ vs PRIMARY: {rho_primary:.6f}")
        print(f"    flips: +→− {flips_to_neg}, −→+ {flips_to_pos}, "
              f"ratio {ratio:.3f}")
        np.save(ART / f"oof_d17_path_b_K23_v4_h1d_tau{tau}_strat.npy",
                np.column_stack([1 - oof, oof]))
        np.save(ART / f"test_d17_path_b_K23_v4_h1d_tau{tau}_strat.npy",
                np.column_stack([1 - tp, tp]))
        sub = sample_sub.copy(); sub[TARGET] = tp
        Path("submissions").mkdir(exist_ok=True)
        sub.to_csv(f"submissions/submission_d17_path_b_K23_v4_h1d_tau{tau}.csv",
                   index=False)
    ...
    (ART / "d17_path_b_K23_v4_h1d_results.json").write_text(
        json.dumps(final, indent=2))
    print(f"\n→ scripts/artifacts/d17_path_b_K23_v4_h1d_results.json  "
