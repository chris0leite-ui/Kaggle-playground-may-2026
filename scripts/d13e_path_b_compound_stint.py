"""Day-13 — Compound × Stint hier-meta.

The d13 sweep killed at fold 2 of Compound×Stint. d13's Stint
τ=100000 landed LB 0.95041 (+7bp). Compound×Stint has 24 populated
segments (vs Stint's 5), giving finer compound-specific stint
dynamics. EV +1-3bp incremental.

Tighter constraints to avoid LR convergence stalls on small segments:
  min_rows = 1000 (was 200) — skip segments under 1000 rows
  max_iter = 500  (was 2000) — cap LR iterations
  C = 1.0          — same regularization

Same K=21 PRIMARY pool: POOL_KEEP (16) + TOP_3_D9 (3) + FM_A + FM_B.
Sweep τ ∈ {5000, 20000, 100000, 500000}. ~10 min wall.
"""
...
def main():
    ...
    for tau in taus:
        ...
        print(f"    flips vs Stint:  +→− {flips_w_to_neg}, "
              f"−→+ {flips_w_to_pos}")
        # Save
        np.save(ART / f"oof_d13e_compound_stint_tau{tau}_strat.npy",
                np.column_stack([1 - oof, oof]))
        np.save(ART / f"test_d13e_compound_stint_tau{tau}_strat.npy",
                np.column_stack([1 - tp, tp]))
        sub = sample_sub.copy(); sub[TARGET] = tp
        sub.to_csv(f"submissions/submission_d13e_compound_stint_tau{tau}.csv",
                   index=False)
    ...
    (ART / "d13e_compound_stint_results.json").write_text(
        json.dumps(final, indent=2))
    print(f"\n→ scripts/artifacts/d13e_compound_stint_results.json  "
