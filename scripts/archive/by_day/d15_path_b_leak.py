"""scripts/d15_path_b_leak.py — hier-meta with leak_lookup base.

Probe two configurations under PRIMARY's hier-meta architecture:
  K=22_leak       : K=21 + d15_leak_lookup
  K=23_leak_orig  : K=21 + d15_leak_lookup + d15_orig_transfer

Compares against:
  K=21 hier-meta = 0.95083   (PRIMARY pool, no extras)
  K=22 + orig_transfer = 0.95094 (LB tie 0.95049)

If K=23 OOF > 0.95095 and flips < 200 → HEDGE/PRIMARY candidate.
"""
...
def main():
    ...
    configs = {
        "K22_leak":            POOL_KEEP + ["d15_leak_lookup"],
        "K23_leak_orig":       POOL_KEEP + ["d15_leak_lookup", "d15_orig_transfer"],
    }
    ...
    for label, names in configs.items():
        ...
        d_K22_orig = (auc - 0.95094) * 1e4
        print(f"  OOF: {auc:.5f}  Δ K=21: {d_K21:+.3f}bp  Δ K=22(orig): {d_K22_orig:+.3f}bp")
        print(f"  ρ vs PRIMARY: {rho:.6f}")
        print(f"  flips top-1%: +→− {flips_to_neg}, −→+ {flips_to_pos}, ratio {ratio:.3f}, total {flips_to_neg+flips_to_pos}")
        ...
        results[label] = dict(
            K=len(names), oof=auc, delta_K21_bp=float(d_K21),
            delta_K22_orig_bp=float(d_K22_orig),
    ...
    print("\n=== Summary ===")
    print(f"  Reference: K=21 hier-meta = 0.95083 | K=22 +orig_transfer = 0.95094 (LB 0.95049 TIE)")
    for label, r in results.items():
        print(f"  {label:<22s}  OOF {r['oof']:.5f}  Δ K=22 {r['delta_K22_orig_bp']:+.2f}bp  "
              f"ρ {r['rho_vs_primary']:.5f}  flips {r['flips_to_neg']+r['flips_to_pos']}")

    results["wall_s"] = time.time() - t0
    (ART / "d15_path_b_leak_results.json").write_text(json.dumps(results, indent=2))
