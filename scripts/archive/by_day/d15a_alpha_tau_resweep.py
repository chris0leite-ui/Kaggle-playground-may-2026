"""Day-15 — α from full-train counts at OOF time + expanded τ grid.

Hypothesis: current d13e Compound×Stint hier-meta uses fold-train
segment counts to compute α at OOF time, but full-train counts at
test time. Bayesian-correct asymmetry, but means τ chosen on OOF
may not be τ-optimal for test. Fix: use full-train segment counts
at OOF time too. W_local is still fit on fold-train rows (correct
to avoid leakage), only the shrinkage weight α uses full-train counts.

Expanded τ grid: {2000, 5000, 10000, 20000, 50000, 100000, 200000}.

Pool: same K=21 as d13e (POOL_KEEP + TOP_3_D9 + FM_A + FM_B).
"""
...
def main():
    ...
    # FULL-TRAIN segment counts (used for α at BOTH OOF and test time)
    counts_full_train = np.bincount(seg_train, minlength=n_seg).astype(np.int64)
    print(f"  full-train counts: nonzero segs = "
          f"{int(np.sum(counts_full_train > 0))}, "
          f"≥{MIN_ROWS} = {int(np.sum(counts_full_train >= MIN_ROWS))}")
    ...
    # Hier-meta sweep (expanded τ grid)
    taus = [2000, 5000, 10000, 20000, 50000, 100000, 200000]
    oofs = {tau: np.zeros(len(y)) for tau in taus}

    print("\n--- Compound×Stint hier-meta (α from full-train counts) ---")
    for fold, (tr, va) in enumerate(splits):
        t_fold = time.time()
        ...
        for s in range(n_seg):
            ...
            mask[s] = True
        # KEY CHANGE: α uses full-train counts (not fold-train counts)
        n_local = counts_full_train.astype(np.float64)
        for tau in taus:
    ...
    for s in range(n_seg):
        ...
        mask_full[s] = True
    test_preds = {}
    n_local_full = counts_full_train.astype(np.float64)
    for tau in taus:
        alpha = n_local_full / (n_local_full + tau)
    ...
    # Evaluate each tau
    print("\n=== Compound × Stint sweep (α from full-train) ===")
    ...
    for tau in taus:
        ...
        print(f"    flips vs Stint:  +→− {flips_w_to_neg}, "
              f"−→+ {flips_w_to_pos}")
        # Save (NOT overwriting d13e files)
        np.save(ART / f"oof_d15a_alpha_tau{tau}_strat.npy",
                np.column_stack([1 - oof, oof]))
        np.save(ART / f"test_d15a_alpha_tau{tau}_strat.npy",
    ...
    (ART / "d15a_alpha_tau_resweep_results.json").write_text(
        json.dumps(final, indent=2))
    print(f"\n→ scripts/artifacts/d15a_alpha_tau_resweep_results.json  "
