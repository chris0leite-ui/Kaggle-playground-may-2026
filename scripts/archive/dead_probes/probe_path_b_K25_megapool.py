"""scripts/probe_path_b_K25_megapool.py — Path B over K=22 amplification probe.
...
under Path B Compound×Stint hier-meta on the K=25 pool.
...
EXTRAS = ["inv_laps_until_pit", "d15b_lgbm_dae_only", "target_reform_pit_horizon", "target_reform_reverse_cum"]  # the +1.348 bp ablation winner
...
def main():
    ...
    K = len(base_oofs)
    print(f"K=25 pool ({K21_BASES[-1]} ... + {EXTRAS}); F shape {F_oof.shape}")
    ...
    for tau in TAUS:
        ...
        np.save(ART / f"oof_path_b_K25_megapool_compound_stint_tau{tau}_strat.npy",
                np.column_stack([1 - oof, oof]))
        np.save(ART / f"test_path_b_K25_megapool_compound_stint_tau{tau}_strat.npy",
                np.column_stack([1 - tp, tp]))
        sub = sample_sub.copy(); sub[TARGET] = tp
        sub.to_csv(f"submissions/submission_path_b_K25_megapool_tau{tau}.csv",
                   index=False)
    ...
    out = ART / "probe_path_b_K25_megapool.json"
