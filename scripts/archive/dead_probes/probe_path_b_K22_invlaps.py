"""scripts/probe_path_b_K22_invlaps.py — Path B over K=22 amplification probe.
...
under Path B Compound×Stint hier-meta on the K=22 pool.
...
K22_EXTRA = "inv_laps_until_pit"  # the +1.348 bp ablation winner
...
def main():
    ...
    extra_oof = _pos(ART / f"oof_{K22_EXTRA}_strat.npy")
    extra_test = _pos(ART / f"test_{K22_EXTRA}_strat.npy")
    base_oofs.append(extra_oof); base_tests.append(extra_test)
    F_oof = _expand(np.column_stack(base_oofs))
    F_test = _expand(np.column_stack(base_tests))
    K = len(base_oofs)
    print(f"K=22 pool ({K21_BASES[-1]} ... + {K22_EXTRA}); F shape {F_oof.shape}")
    ...
    for tau in TAUS:
        ...
        np.save(ART / f"oof_path_b_K22_invlaps_compound_stint_tau{tau}_strat.npy",
                np.column_stack([1 - oof, oof]))
        np.save(ART / f"test_path_b_K22_invlaps_compound_stint_tau{tau}_strat.npy",
                np.column_stack([1 - tp, tp]))
        sub = sample_sub.copy(); sub[TARGET] = tp
        sub.to_csv(f"submissions/submission_path_b_K22_invlaps_tau{tau}.csv",
                   index=False)
    ...
    out = ART / "probe_path_b_K22_invlaps.json"
