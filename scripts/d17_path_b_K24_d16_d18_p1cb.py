"""d17 Path-B C×S on K=24 = K=21 + d16_cont_only + d18_chain_decomp + p1_single_cb.

Merges three independent base-add wins of the day:
  - d16_orig_continuous_only      (orig-LGBM on 7 KS-low marginals)        [+3.33 bp K=21+1]
  - d18_chain_decomp              (per-step orig-log-likelihood / chain)    [+7.37 bp K=21+1]
  - p1_single_cb_v3_gpu           (research-backed CB, Bernoulli + native CTR) [+12.06 bp K=21+1]

K=23 (K=21 + d16 + d18) Path-B τ=20k landed LB 0.95149 (+60 bp over yesterday).
K=22 (K=21 + p1_single_cb) Path-B τ=20k landed LB 0.95143 (+54 bp over yesterday, today).

Ranges of d-values predict positive K=24 lift IFF the three add-bases are
mechanistically orthogonal:
  ρ(d18, p1cb) and ρ(d16, p1cb) — TBD; d18 lift is logit-routed (|w_logit|=0.52)
  while p1_single_cb is raw+rank+logit-balanced (|w|=1.46), suggesting
  partial orthogonality.

Outputs:
  oof_d17_path_b_K24_d16_d18_p1cb_tau{tau}_strat.npy  + test_..._strat.npy
  d17_path_b_K24_d16_d18_p1cb_results.json
  submissions/submission_d17_path_b_K24_d16_d18_p1cb_tau{tau}.csv
"""
...
TARGET = "PitNextLap"
SEED, N_FOLDS = 42, 5
PRIMARY_LB_OOF = 0.95184  # current PRIMARY: d18 K=23 d16+d18, LB 0.95149
MIN_ROWS = 1000
MAX_ITER = 500

# K=21 PRIMARY pool (matches d13e/d18 exactly)
POOL_KEEP = [
...
def main():
    ...
    for label, fname in POOL_KEEP + TOP_3_D9 + FM_PAIR + ADDS:
    ...
    print(f"K={len(base_oofs)} bases  ({names[-3:]} are the day-17/18 adds)")
    ...
    print(f"  Global K=24 LR meta OOF: {auc_global:.5f}")

    taus = [5000, 20000, 100000, 500000]
    oofs = {tau: np.zeros(len(y)) for tau in taus}

    print("\n--- Compound×Stint hier-meta on K=24 ---")
    for fold, (tr, va) in enumerate(splits):
        t_fold = time.time()
    ...
    # PRIMARY = d18 K=23 (LB 0.95149) — load test for ρ + flips
    ...
    print(f"\n=== d17 Path-B K=24 sweep (vs d18 K=23 OOF 0.95184 / LB 0.95149) ===")
    ...
    for tau in taus:
        ...
        print(f"\n  τ={tau}:")
        print(f"    Strat OOF: {auc:.5f}  Δ vs d18 OOF: {d_oof_primary:+.2f}bp  "
              f"Δ vs K=24 global: {d_oof_global:+.2f}bp")
        print(f"    ρ vs d16-cont test: {rho_primary:.6f}")
        print(f"    flips: +→− {flips_to_neg}, −→+ {flips_to_pos}, "
              f"ratio {ratio:.3f}")
        np.save(ART / f"oof_d17_path_b_K24_d16_d18_p1cb_tau{tau}_strat.npy",
                np.column_stack([1 - oof, oof]))
        np.save(ART / f"test_d17_path_b_K24_d16_d18_p1cb_tau{tau}_strat.npy",
                np.column_stack([1 - tp, tp]))
        sub = sample_sub.copy(); sub[TARGET] = tp
        Path("submissions").mkdir(exist_ok=True)
        sub.to_csv(f"submissions/submission_d17_path_b_K24_d16_d18_p1cb_tau{tau}.csv",
                   index=False)
    ...
    (ART / "d17_path_b_K24_d16_d18_p1cb_results.json").write_text(
