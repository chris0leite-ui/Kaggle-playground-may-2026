...
y = train_s[d9e.TARGET].astype(int).values
...
t0 = time.time()
Xtr_csr, Xte_csr = d9e.build_main_effect_hashes(train_s, test, tr)
idx_tr_full = d9e.csr_to_index_array(Xtr_csr)
idx_te = d9e.csr_to_index_array(Xte_csr)
print(f"feat-build wall {time.time()-t0:.1f}s; shapes "
      f"train_idx {idx_tr_full.shape} test_idx {idx_te.shape}")
params = dict(embed_dim=4, epochs=4, lr=0.05, batch=8192)
p_va, p_te = d9e.fit_ffm_one_fold(idx_tr_full, idx_te, y, tr, va, params, seed=42)
