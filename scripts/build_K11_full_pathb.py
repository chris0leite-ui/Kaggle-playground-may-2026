"""Build K=11 = K=4 forward-greedy + all six slim nearest-neighbour bases
+ K=27 super-base under per-segment shrinkage (Path-B C x Stint) at tau=100k.

Reproduces the historical leaderboard 0.95385 ingredient — the K=11 layer
of the current PRIMARY (which was a 70/30 rank-blend of this K=11 stack
and the K=9 slim-nearest-neighbour-only stack).

Today's K=8 submission landed at leaderboard 0.95382 (cross-validation
0.95442, +1.055 bp vs K=27+Path-B, 1.3x transfer ratio). Adding the three
remaining nearest-neighbour bases (qAA, qAF, qAK) should add another
0.4-0.6 bp at cross-validation and roughly 0.5-0.8 bp at the leaderboard
under the same transfer ratio.

Outputs:
  artifacts/K11_full_pathb_tau100000_oof.npy
  artifacts/K11_full_pathb_tau100000_test.npy
  artifacts/K11_full_pathb_tau100000.json
  artifacts/submission_K11_full_pathb_tau100000.csv
"""
...
def main() -> None:
    ...
    K4_oof_anchor = lr_meta_oof(expand(np.column_stack(all_oof[:4])), y)
    K4_auc = float(roc_auc_score(y, K4_oof_anchor))
    print(f"\nK=11 plain LR-meta OOF: {plain_auc:.5f}  (vs K=4 anchor {K4_auc:.5f}, delta {(plain_auc-K4_auc)*1e4:+.3f} bp)", flush=True)

    K4_pathb_oof = _pos(ART / "oof_K4_fwd_pathb.npy")
    K4_pathb_auc = float(roc_auc_score(y, K4_pathb_oof))
    K27_pathb_oof = _pos(ART / "oof_d18_path_b_K27_v4h1d_d16_d18_e2_f2_tau100000_strat.npy")
    K27_pathb_auc = float(roc_auc_score(y, K27_pathb_oof))
    K8_pathb_oof_path = ART / "K8_qAT_qAV_qAO_K27_pathb_tau100000_oof.npy"
    K8_pathb_auc = float(roc_auc_score(y, _pos(K8_pathb_oof_path))) if K8_pathb_oof_path.exists() else None
    print(f"K=4 + Path-B reference:   {K4_pathb_auc:.5f}", flush=True)
    print(f"K=27 + Path-B reference:  {K27_pathb_auc:.5f}", flush=True)
    if K8_pathb_auc is not None:
        print(f"K=8 + Path-B reference:   {K8_pathb_auc:.5f}", flush=True)

    print(f"\n--- Running Path-B C x Stint tau={TAU} on K=11 ---", flush=True)
    oof, test_pred = run_pathb(all_oof, all_test, train, test, y, tau=TAU)
    auc = float(roc_auc_score(y, oof))
    print(f"\nK=11 + Path-B OOF AUC:     {auc:.5f}", flush=True)
    print(f"  delta vs K=4 + Path-B:    {(auc - K4_pathb_auc) * 1e4:+.3f} bp", flush=True)
    print(f"  delta vs K=27 + Path-B:   {(auc - K27_pathb_auc) * 1e4:+.3f} bp", flush=True)
    if K8_pathb_auc is not None:
        print(f"  delta vs K=8 + Path-B:    {(auc - K8_pathb_auc) * 1e4:+.3f} bp", flush=True)

    rho_oof = float(spearmanr(oof, K27_pathb_oof).statistic)
    ...
    print(f"\nrho_oof  vs K=27 + Path-B: {rho_oof:.6f}", flush=True)
    print(f"rho_test vs K=27 + Path-B: {rho_test:.6f}", flush=True)
    if K8_pathb_auc is not None:
        K8_pathb_test = _pos(ART / "K8_qAT_qAV_qAO_K27_pathb_tau100000_test.npy")
        rho_oof_K8 = float(spearmanr(oof, _pos(K8_pathb_oof_path)).statistic)
        rho_test_K8 = float(spearmanr(test_pred, K8_pathb_test).statistic)
        print(f"rho_oof  vs K=8 + Path-B:  {rho_oof_K8:.6f}", flush=True)
        print(f"rho_test vs K=8 + Path-B:  {rho_test_K8:.6f}", flush=True)
    verdict = "ABORT_TIE" if rho_test > 0.999 else "OK_TO_SUBMIT"
    print(f"Verdict (vs K=27): {verdict}", flush=True)

    np.save(ART / f"K11_full_pathb_tau{TAU}_oof.npy", oof)
    np.save(ART / f"K11_full_pathb_tau{TAU}_test.npy", test_pred)
    sub = pd.DataFrame({"id": test["id"], "PitNextLap": test_pred})
    csv_path = ART / f"submission_K11_full_pathb_tau{TAU}.csv"
    ...
    summary = {
        "bases": names,
        "tau": TAU,
        "K4_anchor_plain_oof": K4_auc,
        "K11_plain_oof": plain_auc,
        "K4_pathb_oof": K4_pathb_auc,
        "K27_pathb_oof": K27_pathb_auc,
        "K8_pathb_oof": K8_pathb_auc,
        "K11_pathb_oof": auc,
        "delta_vs_K4_pathb_bp": (auc - K4_pathb_auc) * 1e4,
        "delta_vs_K27_pathb_bp": (auc - K27_pathb_auc) * 1e4,
        "delta_vs_K8_pathb_bp": (auc - K8_pathb_auc) * 1e4 if K8_pathb_auc is not None else None,
        "rho_oof_vs_K27_pathb": rho_oof,
        "rho_test_vs_K27_pathb": rho_test,
        "verdict": verdict,
        "csv": csv_path.name,
        "elapsed_sec": time.time() - t0,
    }
    (ART / f"K11_full_pathb_tau{TAU}.json").write_text(
