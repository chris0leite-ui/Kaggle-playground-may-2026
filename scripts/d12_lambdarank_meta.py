"""Day-12 — Option 5: metric-aligned LambdaRank meta over the K=21 PRIMARY pool.

Hypothesis: the K=21 stack uses BCE-trained bases + LR-meta (squared/logloss).
The competition metric is AUC (rank-based). Replacing LR-meta with a
LambdaMART (LightGBM lambdarank) or CatBoost YetiRank meta — both
pairwise rank-based losses aligned with AUC — could shift the test
ranking by 2-8bp.

Test BOTH grouping schemes:
  - Race-grouped (matches the natural domain group; tests if local
    within-race ranking transfers to global AUC).
  - Random groups of size ~1000 (better aligns lambdarank's pairwise
    objective with the global AUC the competition scores; pairs are
    drawn approximately from the full distribution).

Plus: the existing LR-meta baseline for direct comparison.

Pool (matches scripts/d9f_multi_fm.py K=21 swap PRIMARY):
  POOL_KEEP (16) + TOP_3_D9 (3) + d9f_FM_A + d9f_FM_B = 21.

Strat-only (R1). 5-fold OOF + refit on full data for test prediction.
"""
...
TARGET = "PitNextLap"
SEED, N_FOLDS = 42, 5
PRIMARY_S = 0.95073   # d9f K21_swap_partA_partB OOF
PRIMARY_LB = 0.95031  # d9f K21_swap_partA_partB LB
...
MAX_QUERY_LEN = 8000  # LightGBM hard-caps lambdarank queries at 10000 rows.


def build_groups(group_ids, idx, max_query_len=MAX_QUERY_LEN):
    """Build LightGBM lambdarank `group` array.

    LightGBM expects sorted contiguous groups and caps each group at
    10000 rows. We sort idx by group_ids[idx] and split any contiguous
    run > max_query_len into ceil(n/max_query_len) sub-groups.
    """
    g_sub = group_ids[idx]
...
def build_random_groups(idx, group_size=1000, seed=SEED):
    """Random groups of approximately `group_size`. Returns
    (counts, perm) so X[perm] / y[perm] / group=counts is contiguous.
    """
    ...
    base = n // n_groups
    rem = n - base * n_groups
    counts = np.array([base + (1 if g < rem else 0)
                       for g in range(n_groups)], dtype=np.int32)
    return counts, sorted_idx


def fit_lambdarank_meta(F_oof, F_test, y, group_builder,
                        params_profile="default", label="LM"):
    """5-fold LambdaMART meta. group_builder(idx) -> (counts, perm)."""
    if params_profile == "default":
        params = dict(
        ...
        raise ValueError(params_profile)
    ...
    for k, (tr, va) in enumerate(skf.split(np.zeros(len(y)), y)):
        ...
        print(f"  [{label}] fold {k}: iters={biters[-1]} "
              f"AUC={roc_auc_score(yva, p_va):.5f}")
    return meta_oof, test_pred, biters


def fit_yetirank_meta(F_oof, F_test, y, group_ids_global,
                      iterations=600, lr=0.05, depth=5, label="YT"):
    """5-fold CatBoost YetiRank meta. group_ids_global indexes into full data;
    we sort by group_id within each fold (CatBoost requires contiguous groups).
    """
    ...
    for k, (tr, va) in enumerate(skf.split(np.zeros(len(y)), y)):
        # Sort tr/va by group_id (CatBoost requires queries contiguous).
        tr_perm = tr[np.argsort(group_ids_global[tr], kind="stable")]
        va_perm = va[np.argsort(group_ids_global[va], kind="stable")]
        tr_pool = Pool(F_oof32[tr_perm], y[tr_perm],
                       group_id=group_ids_global[tr_perm])
        va_pool = Pool(F_oof32[va_perm], y[va_perm],
                       group_id=group_ids_global[va_perm])
        m = CatBoost({
        ...
        # Predict on the val fold rows in original order (use va, not va_perm,
        # since we just need scores per-row; predict() doesn't depend on group_id
        # at inference).
        meta_oof[va] = m.predict(F_oof32[va])
        test_pred += m.predict(F_test32) / N_FOLDS
        biters.append(int(m.tree_count_))
        print(f"  [{label}] fold {k}: iters={biters[-1]} "
              f"AUC={roc_auc_score(y[va], meta_oof[va]):.5f}")
    return meta_oof, test_pred, biters
...
def main():
    ...
    names = base_names + d9_names + fm_names
    assert len(Xs_oof) == 21, f"K should be 21, got {len(Xs_oof)}"

    P_oof = np.column_stack(Xs_oof)
    P_test = np.column_stack(Xs_test)
    F_oof = expand(P_oof)
    F_test = expand(P_test)
    print(f"K={len(Xs_oof)}  F_oof={F_oof.shape}  F_test={F_test.shape}")
    ...
    # group builders
    def gb_race(idx):
        return build_groups(race_ids_train, idx)

    def gb_random_1000(idx):
        return build_random_groups(idx, group_size=1000, seed=SEED)
    ...
    print(f"  Strat OOF: {lr_auc:.5f}  Δ PRIMARY {(lr_auc - PRIMARY_S)*1e4:+.2f}bp"
          f"  ρ vs PRIMARY: {lr_rho_prim:.5f}  wall {time.time()-t:.1f}s")
    results["lr_meta"] = dict(strat_oof=lr_auc,
                              delta_primary_bp=(lr_auc - PRIMARY_S) * 1e4,
                              rho_vs_primary_test=float(lr_rho_prim))

    # === LambdaMART (Race-grouped, default depth) ===
    print("\n=== LambdaMART meta (Race-grouped, default) ===")
    t = time.time()
    lm_oof, lm_test, biters_lm = fit_lambdarank_meta(
        F_oof, F_test, y, gb_race, "default", "LM-Race")
    lm_auc = float(roc_auc_score(y, lm_oof))
    lm_rho_prim, _ = spearmanr(lm_test, primary_test)
    lm_rho_lr_test, _ = spearmanr(lm_test, lr_test)
    print(f"  Strat OOF: {lm_auc:.5f}  Δ PRIMARY {(lm_auc - PRIMARY_S)*1e4:+.2f}bp")
    print(f"  ρ vs PRIMARY test: {lm_rho_prim:.5f}  ρ vs LR-meta test: {lm_rho_lr_test:.5f}")
    print(f"  best_iters: {biters_lm}  wall {time.time()-t:.1f}s")
    results["lambdamart_race"] = dict(
        strat_oof=lm_auc,
        delta_primary_bp=(lm_auc - PRIMARY_S) * 1e4,
        rho_vs_primary_test=float(lm_rho_prim),
        rho_vs_lr_test=float(lm_rho_lr_test),
        best_iters=biters_lm,
    )

    # === LambdaMART (Random groups of ~1000) ===
    print("\n=== LambdaMART meta (random groups ~1000) ===")
    t = time.time()
    lmr_oof, lmr_test, biters_lmr = fit_lambdarank_meta(
        F_oof, F_test, y, gb_random_1000, "default", "LM-Rand")
    lmr_auc = float(roc_auc_score(y, lmr_oof))
    lmr_rho_prim, _ = spearmanr(lmr_test, primary_test)
    lmr_rho_lr_test, _ = spearmanr(lmr_test, lr_test)
    print(f"  Strat OOF: {lmr_auc:.5f}  Δ PRIMARY {(lmr_auc - PRIMARY_S)*1e4:+.2f}bp")
    print(f"  ρ vs PRIMARY test: {lmr_rho_prim:.5f}  ρ vs LR-meta test: {lmr_rho_lr_test:.5f}")
    print(f"  best_iters: {biters_lmr}  wall {time.time()-t:.1f}s")
    results["lambdamart_rand1000"] = dict(
        strat_oof=lmr_auc,
        delta_primary_bp=(lmr_auc - PRIMARY_S) * 1e4,
        rho_vs_primary_test=float(lmr_rho_prim),
        rho_vs_lr_test=float(lmr_rho_lr_test),
        best_iters=biters_lmr,
    )

    # === YetiRank (Race-grouped, smaller config) ===
    print("\n=== YetiRank meta (Race-grouped) ===")
    t = time.time()
    yt_oof, yt_test, biters_yt = fit_yetirank_meta(
        F_oof, F_test, y, race_ids_train, iterations=400, lr=0.05, depth=5,
        label="YT-Race")
    yt_auc = float(roc_auc_score(y, yt_oof))
    yt_rho_prim, _ = spearmanr(yt_test, primary_test)
    yt_rho_lr_test, _ = spearmanr(yt_test, lr_test)
    print(f"  Strat OOF: {yt_auc:.5f}  Δ PRIMARY {(yt_auc - PRIMARY_S)*1e4:+.2f}bp")
    print(f"  ρ vs PRIMARY test: {yt_rho_prim:.5f}  ρ vs LR-meta test: {yt_rho_lr_test:.5f}")
    print(f"  best_iters: {biters_yt}  wall {time.time()-t:.1f}s")
    results["yetirank_race"] = dict(
        strat_oof=yt_auc,
        delta_primary_bp=(yt_auc - PRIMARY_S) * 1e4,
        rho_vs_primary_test=float(yt_rho_prim),
        rho_vs_lr_test=float(yt_rho_lr_test),
        best_iters=biters_yt,
    )

    # Save Part C artifacts: meta-only swap (K=21 unchanged)
    saves = [
        ("lr_meta", lr_oof, lr_test),
        ("lambdarank_meta", lm_oof, lm_test),       # canonical alias for lambdarank
    ...
    print("→ wrote 3 d12 submissions (NOT to be submitted; per task constraints)")

    final = dict(
        results=results,
        best_meta_by_oof=best_name,
