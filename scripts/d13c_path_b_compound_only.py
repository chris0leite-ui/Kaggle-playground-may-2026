"""Day-13 Path B finalizer (Compound only) — calibration probe.

Compound τ=100000 was the only ρ ≥ 0.999 variant in d13's sweep
(+0.30bp OOF, ρ=0.9990). The killed d13 run never saved test preds.
This rebuilds Compound only, ~3 min wall.
"""
...
def main():
    ...
    seg_train = train["Compound"].astype(str).map(cmp).astype(int).values
    seg_test = test["Compound"].astype(str).map(cmp).astype(int).values
    n_seg = len(cats)
    print(f"Compound segmentation: {n_seg} levels {cats}")
    ...
    for fold, (tr, va) in enumerate(splits):
        ...
        for s in np.unique(seg_train[va]):
            ...
            oof[va[idx]] = predict_aug(F_oof[va[idx]], w)
    ...
    print(f"\n=== Compound τ=100000 result ===")
    print(f"  Strat OOF: {auc:.5f}  Δ PRIMARY (0.95073) "
          f"{(auc-0.95073)*1e4:+.2f}bp")
    print(f"  ρ vs PRIMARY test:  {rho_test:.6f}")
    print(f"  rare-class flips: + → −  {flips_to_neg}, "
          f"− → +  {flips_to_pos}, ratio {ratio:.3f}")

    np.save(ART / "oof_d13_path_b_compound_tau100000_strat.npy",
            np.column_stack([1 - oof, oof]))
    np.save(ART / "test_d13_path_b_compound_tau100000_strat.npy",
            np.column_stack([1 - tp, tp]))
    sub = sample_sub.copy(); sub[TARGET] = tp
    sub.to_csv("submissions/submission_d13_path_b_compound_tau100000.csv",
               index=False)
    final = dict(
        oof=auc, delta_primary_bp=float((auc - 0.95073) * 1e4),
        rho_vs_primary=float(rho_test),
        flip_ratio=float(ratio),
        flips_to_neg=flips_to_neg, flips_to_pos=flips_to_pos,
        wall_s=time.time() - t0,
    )
    (ART / "d13c_path_b_compound_results.json").write_text(json.dumps(final, indent=2))
    print(f"\n→ submissions/submission_d13_path_b_compound_tau100000.csv")
