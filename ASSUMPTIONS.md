...
|---|---|---|---|---|---|
| A1 | PRIMARY (active, set 2026-05-08 PM) is K=4 forward-greedy + Path-B Compound × Stint, τ=100k. OOF 0.95403, LB 0.95351. Bases: yekenot-RealMLP, CatBoost-yekenot, HGBC deep, LightGBM-on-original. The prior K=27 PRIMARY (LB 0.95368) is hedge-eligible per Rule R7 | MEASURED | live | `state/current.md` "PRIMARY (active)" section; `submissions/submission_K4_fwd_pathb.csv` LB 0.95351 confirmed via `kaggle competitions submissions`; `scripts/artifacts/oof_K4_fwd_pathb_strat.npy` and `test_K4_fwd_pathb_strat.npy` are the operational OOF/test artefacts | 2026-05-08 |
| A1b | The prior K=27 PRIMARY artefact and its hedge-eligibility row remain available. If a future session needs the higher-LB-score reference, use `submissions/submission_PRIMARY_d18_K27_pathb_tau100k.csv` (LB 0.95368) | MEASURED | live | Same artefact as A26's reference. Held in `submissions/` for hedge use | 2026-05-08 |
...
| A25 | The K=27 pool's effective rank is **3.23 in logit, 1.63 in probability** — the same dimensionality as K=24 was. The d18 "DGP-class" bases (e2_preimage_knn, f2_constraint, chain_decomp) added ~0.35 to logit eff-rank, not a new direction | MEASURED | live (refined; see A25b) | `scripts/probe_pool_structure.py`. Top 5 logit components capture 93% variance. Top redundant pair: f2_constraint ↔ HGBC-class at ρ=+0.985 — f2_constraint is structurally an HGBC clone, not a "DGP" base | 2026-05-08 |
| A25b | A25's 3.23 is **variance-effective-rank**, not predictive-effective-rank. The PCA-meta probe shows top-3 PCA-LR scores only 0.95061 (−35.64 bp vs K=10+1 plain LR 0.95417); top-15 PCA-LR scores 0.95401 (≈anchor); top-27 PCA-LR scores 0.95422. The LR-meta uses **~15 principal components of predictive content**, not 3. Low-variance PCs carry meaningful signal about the binary target — canonical "supervised methods don't necessarily prefer high-variance directions" | MEASURED | live (NEW) | `scripts/probe_pca_meta.py` Variant A; `audit/2026-05-08-pca-meta-probe.md`. Refines but does not falsify A30 — the LR-meta family ceiling is real, just on a 15-D effective manifold not 3-D | 2026-05-08 |
...
| A30 | The 3-D logit subspace of the K=10 / K=27 pool is the **information ceiling of the 14-feature row-level data under the LR-meta family**. Breaking it requires either (i) data beyond the 14 columns, or (ii) a non-LR meta architecture | FALSIFIED | dropped (see A30b/A30c) | A29 + A25 + EXP-2/3/4 NULL motivated this. **Two independent falsifications on 2026-05-08 PM:** (1) `research-model-extensions-Ibwvn` tested option (ii) on K=4: GBM meta −1.20 bp, 2-hidden-layer MLP meta −7.77 bp, LR + raw row features flat at −0.04 bp; (2) `pca-k25-ensemble-cUKNt` tested option (ii) on K=27 PCA: LightGBM-meta worse than LR at every PCA truncation and at K=27+1 raw. Non-LR meta overfits the meta projection on a low-eff-rank logit pool (A31 at K=4, A25 at K=27). The information ceiling is real but the route around it is NOT "non-LR meta." Audits: `audit/2026-05-08-four-lane-research-extension.md`, `audit/2026-05-08-pca-meta-probe.md`, `scripts/artifacts/probe_lane4_nonlr_meta.json`, `scripts/artifacts/probe_pca_meta.json` | 2026-05-08 |
| A30b | LightGBM as meta-learner is *worse* than LR at every K=27 input representation: PCA top-K for K∈{3,5,10,15,27} (Δ −1.91 to −32.88 bp vs K=10 plain LR), K=10 raw [P, rank, logit] expansion (−1.25 bp), K=27 raw expansion (+0.01 bp = anchor-tying NULL). Best LightGBM 0.95417; best LR 0.95428 (K=27+1). LightGBM hyperparameters not tuned, but the consistency of the gap across representations argues against a hyperparameter explanation. Mechanism: 27-base predictions are highly redundant (logit eff-rank=3.23) and near-calibrated; bias-variance tradeoff favours LR | MEASURED | live (NEW) | `scripts/probe_pca_meta.py` Variant B; `audit/2026-05-08-pca-meta-probe.md`. Cross-confirms `non-LR-meta-on-K4-regresses` from `research-model-extensions-Ibwvn` — two independent K-pool sizes, two non-LR meta classes, all negative | 2026-05-08 |
| A30c | Path-B Compound × Stint shrinkage requires **base correlations** (not just any feature redundancy) to fire. Path-B on top-K PCs of the K=27 logit pool scores 0.95077-0.95133 (NULL by 28-34 bp vs K=10 plain LR). PCs are uncorrelated by construction, so per-segment slopes can't re-route which base-cluster speaks for that segment | MEASURED | live (NEW) | `scripts/probe_pca_meta.py` Variant C. Clean falsification of the "Path-B finds value via correlation exploitation" intuition — value with correlation, no value without. Refines `state/hypothesis-board.md` "per-segment shrinkage trick fires on a redundant pool" | 2026-05-08 |
| A31 | The K=4 forward-greedy pool's logit effective rank is **1.33** (entropy on singular values), much tighter than K=27's 3.23. Component 1 alone captures 93.6% of variance and correlates with TyreLife/LapNumber/Compound — i.e. "tyre-degradation pressure × compound" is the dominant direction | MEASURED | live (NEW) | `scripts/probe_lane4_nonlr_meta.py` D4.1; SVD on K=4 logits. Singular values [3923, 870, 409, 346]; variance explained [0.94, 0.05, 0.01, 0.01]. Component 1 loadings (-0.56, -0.50, -0.50, -0.44) — consensus direction. Component 2 (4.6%) is "orig-vs-synth" axis (d16_orig loads -0.89, the three synth-trained bases positive). Component 3 (1.0%) is "NN-vs-HGBC" architectural disagreement. **Forward-greedy reduces effective rank faster than base count** | 2026-05-08 |
| A32 | The W3 downsampling marginal is real but FULLY ABSORBED by the K=4 pool. P(pit \| gap=1) = 8.5% vs P(pit \| gap≥11) = 30% (3.5× gradient), but per-gap K=4 LR meta calibration ECE = 0.0001–0.0015 (near-perfect). Gap as meta feature +0.02 bp; gap as base-LGBM feature K=4+1 +0.001 bp; per-gap isotonic −2.18 bp | MEASURED | live (NEW) | `scripts/probe_lane1_downsampling.py` D1.1/D1.2/P1.1/P1.3; `scripts/probe_lane1_gap_base.py`. Closes W3 as an actionable axis: TyreLife + Stint + LapNumber + Compound implicitly carry the gap information | 2026-05-08 |
| A33 | PitNextLap is a **probabilistic** forward-looking label, NOT a deterministic function of observed-row structure. Only 29% of train rows have lap L+1 observed; even there, PitStop[L+1] matches PitNextLap[L] only 81% of the time. The synth's PitNextLap construction has irreducible noise that bounds achievable AUC near where K=4 already sits (~0.954) | MEASURED | live (NEW) | Inline probe during `probe_lane1_gap_base.py` design. Implication: discrete-time-hazard reformulations against `(stint ends in next k laps)` don't cleanly map to PitNextLap. Concrete numbers: agreement rate 0.8094 when L+1 observed; P(PitNextLap=1 \| PitStop_at_L+1=1) = 0.18, P(PitNextLap=1 \| PitStop_at_L+1=0) = 0.11 | 2026-05-08 |
| A34 | Per-Compound K=4 PRIMARY calibration is well-calibrated except for WET (n=1,355): WET ECE = 0.0112, 3-7× higher than other compounds. p̂_mean=0.017 vs ȳ_mean=0.025 (PRIMARY underpredicts WET pits). However the segment is too small for any per-Compound meta head to recover — per-Compound LR routing +0.11 bp, rain-row blend +0.03 bp | MEASURED | live (NEW) | `scripts/probe_lane3_routing.py` D3.2/P3.1/P3.3. W1 (rain residual) survives this lane | 2026-05-08 |
