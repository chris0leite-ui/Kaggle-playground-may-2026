...
| A30 | The 3-D logit subspace of the K=10 / K=27 pool is the **information ceiling of the 14-feature row-level data under the LR-meta family**. Breaking it requires either (i) data beyond the 14 columns, or (ii) a non-LR meta architecture | INFERRED | refined (see A30b/A30c) | A29 + A25 + EXP-2/3/4 NULL across 3 distinct task framings. The inference is strong: every task framing, model class, and feature engineering recipe at row level has been tested null at K=10+1. Doesn't rule out a nonlinear meta-projection (gradient boosting on predictions or neural meta-learner) — that's the only architecturally untested avenue | 2026-05-08 |
| A30b | The "non-LR meta architecture" clause of A30 is **empirically REFUTED**. LightGBM as meta-learner is *worse* than LR at every input representation tested: PCA top-K for K∈{3,5,10,15,27} (Δ −1.91 to −32.88 bp), K=10 raw [P, rank, logit] expansion (−1.25 bp), K=27 raw expansion (+0.01 bp = anchor-tying NULL). Best LightGBM 0.95417; best LR 0.95428 (K=27+1). LightGBM hyperparameters not tuned, but the consistency of the gap across representations argues against a hyperparameter explanation. Plausible mechanism: 27-base predictions are highly redundant (logit eff-rank=3.23) and near-calibrated; bias-variance tradeoff favours LR | MEASURED | live (NEW) | `scripts/probe_pca_meta.py` Variant B; `audit/2026-05-08-pca-meta-probe.md`. The only structurally open clause of A30 closes negative — remaining options are (i) external data (PI ruled out) or acceptance posture | 2026-05-08 |
