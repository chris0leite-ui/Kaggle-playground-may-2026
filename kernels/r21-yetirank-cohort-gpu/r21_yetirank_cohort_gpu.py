...
def main():
    ...
    # Save outputs
    np.save(WORK / "oof_R21_yetirank_yrl_strat.npy",
            oof_meta.astype(np.float32))
    np.save(WORK / "test_R21_yetirank_yrl_strat.npy",
            test_meta.astype(np.float32))
    print(f"[save] wrote oof_R21_yetirank_yrl_strat.npy + test_..._strat.npy",
          flush=True)

    summary = dict(
        round="R21_yetirank_yrl_gpu",
        oof_auc=auc_full,
        fold_aucs=fold_aucs,
        fold_walls_s=walls,
    ...
    (WORK / "r21_yetirank_summary.json").write_text(json.dumps(summary, indent=2))
    print(f"[save] wrote r21_yetirank_summary.json  "
