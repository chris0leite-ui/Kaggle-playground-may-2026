"""TabPFN-2.6 fine-tuning on Kaggle GPU — Strat fold-0 probe (Day-13).

TabPFN v2.6 uses the same FinetunedTabPFNClassifier API as v2.5 but the
_create_estimator method is hardcoded to ModelVersion.V2_5. We monkey-patch
...
Outputs (under /kaggle/working/):
  oof_d13_tabpfn_v26_strat.npy   shape (n_train, 2)
  test_d13_tabpfn_v26_strat.npy  shape (n_test, 2)
  d13_tabpfn_v26_results.json
"""
...
def main():
    ...
    # Monkey-patch _create_estimator to load TabPFN v2.6 weights instead of
    # the hardcoded V2_5. Prior-Labs/tabpfn_2_6 on HuggingFace.
    def _create_estimator_v26(self, config):
    ...
    FinetunedTabPFNClassifier._create_estimator = _create_estimator_v26
    print("[setup] FinetunedTabPFNClassifier monkey-patched -> ModelVersion.V2_6")
    ...
    for k, (tr, va) in enumerate(splits_strat):
        ...
        np.save(WORK / "oof_d13_tabpfn_v26_strat.npy",
                np.column_stack([1 - oof, oof]))
        np.save(WORK / "test_d13_tabpfn_v26_strat.npy",
    ...
    sub.to_csv(WORK / "submission_d13_tabpfn_v26.csv", index=False)

    res = dict(
        model="TabPFN-2.6_finetuned",
        oof_score=auc_full,
        ...
        notes=("TabPFN-2.6 fine-tuned (gradient updates, NOT zero-shot ICL)."
    ...
    (WORK / "d13_tabpfn_v26_results.json").write_text(json.dumps(res, indent=2))
