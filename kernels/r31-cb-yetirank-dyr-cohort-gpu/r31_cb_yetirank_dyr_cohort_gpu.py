"""R31 inventor — CatBoost YetiRank META on K=17 with (Driver, Year, Race)
cohort grouping. Sibling of R21 (CB YetiRank on (Y,R,L)): same model family
and loss, fundamentally different cohort definition.

Why this cohort: (Driver, Year, Race) groups a single driver's lap sequence
within a single race. The listwise loss ranks laps by pit probability
*within that driver's race*. R21's (Y,R,L) groups all cars on the same
lap; this groups all laps of one car. Sequence-like axis vs lap-snapshot
axis.

Group stats: 40,869 groups, median 11 laps/group, max 38 — well below CB
GPU YetiRank's safe limits.

Output: oof_R31_yetirank_dyr_strat.npy + test_R31_yetirank_dyr_strat.npy
to /kaggle/working/.
"""
...
def main():
    t0 = time.time()
    print("== R31 inventor: CatBoost YetiRank META on (Driver,Year,Race) cohort ==",
          flush=True)
    gpu_boot()
    ...
    for k, (ti, vi) in enumerate(fold_list, 1):
        ...
        sort_idx = np.lexsort((
            ...
            ti_df["Driver"].astype(str).values,
        ))
        ti_df_sorted = ti_df.iloc[sort_idx].reset_index(drop=True)
        X_tr = F_oof[ti][sort_idx]
        y_tr = y_all[ti][sort_idx]
        group_id_tr = (ti_df_sorted["Driver"].astype(str) + "|" +
                       ti_df_sorted["Year"].astype(str) + "|" +
    ...
    print(f"\n[result] R31 YetiRank (Driver,Y,R) cohort OOF AUC: {auc_full:.6f}",
          flush=True)

    np.save(WORK / "oof_R31_yetirank_dyr_strat.npy", oof_meta.astype(np.float32))
    np.save(WORK / "test_R31_yetirank_dyr_strat.npy", test_meta.astype(np.float32))
    print(f"[save] wrote oof_R31_yetirank_dyr_strat.npy + test_..._strat.npy",
          flush=True)

    summary = dict(
        round="R31_yetirank_dyr_gpu",
        cohort="(Driver, Year, Race)",
        oof_auc=auc_full,
        fold_aucs=fold_aucs,
        fold_walls_s=walls,
    ...
    (WORK / "r31_yetirank_dyr_summary.json").write_text(json.dumps(summary, indent=2))
    print(f"[save] wrote r31_yetirank_dyr_summary.json  "
