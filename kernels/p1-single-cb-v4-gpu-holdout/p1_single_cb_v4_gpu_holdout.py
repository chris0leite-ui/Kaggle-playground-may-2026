"""P1 single-CB **v4 holdout** — Rule 24 mandatory check.

Independent 80/20 split (HOLDOUT_SEED=99 != main 5-fold seed=42) for the
v4 recipe (yekenot items 2/3/4/7 added to v3 base). Same FE + recipe
as kernels/p1-single-cb-v4-gpu/, but trains on 80% / eval on 20%.

Pass criterion (Rule 24):
  |holdout_auc - oof_auc_ref| <= 10 bp = clean

oof_auc_ref = 0.95200 (v4 5-fold OOF, commit 79eb71b).

Outputs:
  p1_holdout_v4_gpu_results.json
  oof_holdout_v4_gpu_seed42.npy
  test_holdout_v4_gpu_seed42.npy
"""
...
# v4 holdout
WITH_ORIG_DATA = True       # item 7 (matches parent v4)
N_SEEDS = 1
SEEDS_BAG = [42, 13, 71]
MAX_ROUNDS = 4000
HOLDOUT_SEED = 99           # independent of main 5-fold seed=42
OOF_AUC_REF = 0.95200       # v4 5-fold OOF (commit 79eb71b)
...
def main():
    ...
    # === HOLDOUT MODE: replace 5-fold with single 80/20 split ===
    skf_h = StratifiedKFold(5, shuffle=True, random_state=HOLDOUT_SEED)
    train_idx, holdout_idx = next(skf_h.split(np.zeros(len(y)), y))
    print(f"=== HOLDOUT 80/20 (seed={HOLDOUT_SEED}) | "
          f"train={len(train_idx)} holdout={len(holdout_idx)} ===")
    fold_list = [(train_idx, holdout_idx)]
    # SMOKE flag is irrelevant in holdout mode; force single iteration.
    SMOKE = True  # noqa: makes n_eff_folds = 1 below
    ...
    for fold, (ti, vi) in enumerate(fold_list[:n_eff_folds], 1):
        ...
        # Train self-AUC on the 80% (synth-only portion)
        synth_n = len(train_ti)
        pred_tr = m.predict_proba(X_tr.iloc[:synth_n])[:, 1]
        train_auc = float(roc_auc_score(y_tr[:synth_n], pred_tr))
        best_iter = int(m.tree_count_)
        wall = time.time() - t1
        print(f"\n  holdout AUC = {holdout_auc:.5f}")
        print(f"  train self-AUC = {train_auc:.5f}")
    ...
    if verdict == "FAIL":
        print(f"  → DO NOT submit v4 single-CB to LB. Recipe leak suspected.")
    else:
        print(f"  → v4 recipe is fold-safe. OOF→LB transfer plausible.")

    np.save(WORK / "oof_holdout_v4_gpu_seed42.npy",
            np.column_stack([1 - pred_h, pred_h]).astype(np.float64))
    np.save(WORK / "test_holdout_v4_gpu_seed42.npy",
            np.column_stack([1 - pred_h, pred_h]).astype(np.float64))
    results = dict(
    ...
    (WORK / "p1_holdout_v4_gpu_results.json").write_text(
        json.dumps(results, indent=2, default=str))
    print(f"\nDONE wall={time.time()-t0_total:.0f}s  → "
          f"p1_holdout_v4_gpu_results.json")
