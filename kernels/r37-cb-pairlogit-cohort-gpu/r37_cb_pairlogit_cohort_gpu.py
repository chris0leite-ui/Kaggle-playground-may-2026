"""R37 inventor — CatBoost PairLogit (Lambdarank-equivalent) META on K=17 with
(Year, Race, LapNumber) cohort. Sibling of R21 (YetiRank): same cohort, same
features, DIFFERENT ranking loss family.

YetiRank uses Yandex's smoothed-listwise smoothing; PairLogit uses pairwise
logistic loss (essentially LambdaRank). The optimization landscapes differ,
producing structurally different rank outputs for blend ingredient diversity.

Hypothesis: PairLogit standalone OOF in [0.953, 0.955]; ρ vs R21 < 0.98
would make it a useful 3-way blend ingredient (alternative to R30 5-seed bag).

Output: oof_R37_pairlogit_yrl_strat.npy + test_R37_pairlogit_yrl_strat.npy
to /kaggle/working/.
"""
...
def main():
    t0 = time.time()
    print("== R37 inventor: CatBoost PairLogit META on (Year,Race,LapNumber) cohort ==",
          flush=True)
    gpu_boot()
    ...
    for k, (ti, vi) in enumerate(fold_list, 1):
        ...
        # Sort by (Year, Race, LapNumber) for ranking group_id
        sort_idx = np.lexsort((
        ...
        # Build per-row group_id (integer) — CB needs int, not group-sizes
    ...
    print(f"\n[result] R37 PairLogit cohort OOF AUC: {auc_full:.6f}", flush=True)

    # Save outputs
    np.save(WORK / "oof_R37_pairlogit_yrl_strat.npy",
            oof_meta.astype(np.float32))
    np.save(WORK / "test_R37_pairlogit_yrl_strat.npy",
            test_meta.astype(np.float32))
    print(f"[save] wrote oof_R37_pairlogit_yrl_strat.npy + test_..._strat.npy",
          flush=True)

    summary = dict(
        round="R37_pairlogit_yrl_gpu",
        oof_auc=auc_full,
        fold_aucs=fold_aucs,
        fold_walls_s=walls,
    ...
    (WORK / "r37_pairlogit_summary.json").write_text(json.dumps(summary, indent=2))
    print(f"[save] wrote r37_pairlogit_summary.json  "
