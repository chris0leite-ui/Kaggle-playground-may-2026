...
Output: oof_R29_cb_qce_yrl_strat.npy + test_R29_cb_qce_yrl_strat.npy
...
def main():
    ...
    print(f"\n[result] R21 YetiRank cohort OOF AUC: {auc_full:.6f}", flush=True)

    # Save outputs
    np.save(WORK / "oof_R29_cb_qce_yrl_strat.npy",
            oof_meta.astype(np.float32))
    np.save(WORK / "test_R29_cb_qce_yrl_strat.npy",
            test_meta.astype(np.float32))
    print(f"[save] wrote oof_R29_cb_qce_yrl_strat.npy + test_..._strat.npy",
          flush=True)

    summary = dict(
        round="R29_cb_qce_yrl_gpu",
        oof_auc=auc_full,
        fold_aucs=fold_aucs,
        fold_walls_s=walls,
    ...
    (WORK / "r29_cb_qce_summary.json").write_text(json.dumps(summary, indent=2))
    print(f"[save] wrote r29_cb_qce_summary.json  "
