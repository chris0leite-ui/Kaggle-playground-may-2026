...
def main():
    ...
    # R30 = 5-seed bag of R21 YetiRank architecture
    BAG_SEEDS = [42, 17, 31, 53, 71]
    ...
    np.save(WORK / "oof_R30_r21_bag_yrl_strat.npy", oof_bag.astype(np.float32))
    np.save(WORK / "test_R30_r21_bag_yrl_strat.npy", test_bag.astype(np.float32))
    print("[save] wrote oof_R30_r21_bag_yrl_strat.npy + test_..._strat.npy",
          flush=True)

    summary = dict(round="R30_r21_bag_yrl_gpu", oof_auc=auc_full,
                   bag_seeds=BAG_SEEDS, fold_aucs=all_fold_aucs,
                   wall_total_s=time.time() - t0, bases=names)
    (WORK / "r30_r21_bag_summary.json").write_text(json.dumps(summary, indent=2))
    print(f"[save] wrote r30_r21_bag_summary.json  "
