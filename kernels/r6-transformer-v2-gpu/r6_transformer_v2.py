...
def main():
    ...
    # 5-fold GroupKFold by (Year, Race, Driver) sequence — fixes v1's
    # intra-sequence leakage where val rows came from same sequences as
    # train rows. v2 keeps full sequences intact in one fold.
    train_groups = (train_s["Year"].astype(str) + "_" +
                    train_s["Race"].astype(str) + "_" +
                    train_s["Driver"].astype(str)).values
    gkf = GroupKFold(n_splits=N_FOLDS)
    fold_iter = list(gkf.split(np.zeros(len(y_all)), y_all, groups=train_groups))
    ...
    np.save(f"{out_dir}/oof_r6_transformer_v2_strat.npy", out_oof)
    np.save(f"{out_dir}/test_r6_transformer_v2_strat.npy", out_test)
    with open(f"{out_dir}/r6_transformer_v2_results.json", "w") as f:
        json.dump(dict(name="r6_transformer_v2", oof_auc=auc_full,
