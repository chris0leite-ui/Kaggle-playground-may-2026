...
# P100 (sm_60) compat: Kaggle silently routes GpuT4x2 to P100 nodes; current
# torch (>=2.6) drops sm_60 wheels. Force-reinstall torch 2.4 BEFORE any
# torch usage so SDV's CTGAN/TVAE/CopulaGAN run on sm_60.
...
def main():
    ...
    # Disc-features as PER-ROW base: train discriminator on (orig, replay)
    # then SCORE synth rows. The score answers "is this synth row more
    # orig-like or more replay-like under this architecture?" — meaningful
    # if host_synth was generated by this architecture (then synth ≈ replay
    # → score near 1) vs not (synth somewhere between orig and replay).
    print(f"\n[per-row discriminator features for stack]", flush=True)
    # Encode synth + orig + each replay with a UNION mapping per cat col
    all_cats_union = {}
    for c in FEATS_CAT:
        uni = set(tr[c].astype(str)) | set(te[c].astype(str)) | \
              set(orig[c].astype(str))
        for replay in replays.values():
            uni |= set(replay[c].astype(str))
        all_cats_union[c] = {v: i for i, v in enumerate(sorted(uni))}

    def encode(df_in):
        df_e = df_in[FEATS_NUM + FEATS_CAT].copy()
        for c in FEATS_NUM:
            df_e[c] = pd.to_numeric(df_e[c], errors="coerce").fillna(0.0)
        for c in FEATS_CAT:
            df_e[c] = df_e[c].astype(str).map(all_cats_union[c]).astype(int)
        return df_e

    orig_e = encode(orig)
    tr_e = encode(tr)
    te_e = encode(te)
    all_synth_e = pd.concat([tr_e, te_e], ignore_index=True)

    for name, replay in replays.items():
        replay_e = encode(replay)
        # Train disc on (orig=label 0, replay=label 1).
        X_t = pd.concat([orig_e, replay_e], ignore_index=True)
        y_t = np.concatenate([np.zeros(len(orig_e), dtype=int),
                              np.ones(len(replay_e), dtype=int)])
        m = lgb.LGBMClassifier(n_estimators=300, learning_rate=0.05,
                               num_leaves=63, min_child_samples=50,
                               verbosity=-1, random_state=SEED)
        m.fit(X_t, y_t)
        # Disc training-set AUC sanity (bootstrap-like)
        p_train = m.predict_proba(X_t)[:, 1]
        train_auc = roc_auc_score(y_t, p_train)
        # Score all 627k synth rows: P(replay-like | x).
        # If host_synth ≈ replay (same arch): mean ≈ 0.5+ (synth looks
        # like replay). If not the arch: mean closer to where orig sits
        # along this disc.
        p_synth = m.predict_proba(all_synth_e)[:, 1]
        n_tr = len(tr_e); oof_pos = p_synth[:n_tr]; te_pos = p_synth[n_tr:]
        ...
        results.setdefault(name, {})["disc_orig_vs_replay_train_auc"] = float(train_auc)
        results.setdefault(name, {})["disc_synth_score_mean_train"] = float(oof_pos.mean())
        results.setdefault(name, {})["disc_synth_score_mean_test"] = float(te_pos.mean())
        print(f"  {name:18s}  disc(orig vs replay) train AUC={train_auc:.4f}  "
              f"synth-side P(replay-like) mean tr={oof_pos.mean():.4f}",
              flush=True)
