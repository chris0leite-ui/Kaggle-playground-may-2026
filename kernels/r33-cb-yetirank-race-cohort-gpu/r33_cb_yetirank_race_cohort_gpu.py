"""R33 inventor — CatBoost YetiRank META on K=17 with (Year, Race)
cohort grouping. Sibling of R21 (CB YetiRank on (Y,R,L)): coarsest cohort
yet — whole race as one group.

Why this cohort: (Year, Race) groups all cars and all laps of a single race
into one ranking list. The listwise loss ranks every (car, lap) row by pit
probability *within the race*. R21's (Y,R,L) was per-lap (cross-driver);
R32's (Y,R,Stint) was per-stint; this is per-race (most aggregated).

Group stats: 104 groups, median 4,548 laps/group, max 7,908 — large groups
but YetiRank tolerates this.

Output: oof_R33_yetirank_yr_strat.npy + test_R33_yetirank_yr_strat.npy
to /kaggle/working/.
"""
...
def main():
    t0 = time.time()
    print("== R33 inventor: CatBoost YetiRank META on (Year,Race) cohort ==",
          flush=True)
    gpu_boot()
    ...
    print(f"[cohort] (Y,R) count: {len(sizes)} "
    ...
    print(f"\n[result] R33 YetiRank (Y,R) cohort OOF AUC: {auc_full:.6f}",
          flush=True)

    np.save(WORK / "oof_R33_yetirank_yr_strat.npy", oof_meta.astype(np.float32))
    np.save(WORK / "test_R33_yetirank_yr_strat.npy", test_meta.astype(np.float32))
    print(f"[save] wrote oof_R33_yetirank_yr_strat.npy + test_..._strat.npy",
          flush=True)

    summary = dict(
        round="R33_yetirank_yr_gpu",
        cohort="(Year, Race)",
        oof_auc=auc_full,
        fold_aucs=fold_aucs,
        fold_walls_s=walls,
    ...
    (WORK / "r33_yetirank_yr_summary.json").write_text(json.dumps(summary, indent=2))
    print(f"[save] wrote r33_yetirank_yr_summary.json  "
