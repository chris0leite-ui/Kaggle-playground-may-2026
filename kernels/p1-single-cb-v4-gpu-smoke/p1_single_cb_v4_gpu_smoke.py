"""P1 single-CB v4 GPU — yekenot transfer + orig augmentation.

Transfer items from research branch's d17_h1d_yekenot_full (LB 0.95345
contributor). v3 had items 1,5,6 already; v4 adds 2,3,4,7:

  ITEM 2 (floor-cat):  np.floor(x).factorize() for ratios + key cont.
                       (LapNumber, TyreLife, RaceProgress, LapTime,
                       Cumulative_Degradation, Position).
  ITEM 3 (count enc):  value_counts() lookup per categorical (Driver,
                       Race, Compound, Year, Stint + combo cats) as
                       NUMERIC features the CB sees directly.
  ITEM 4 (KBins):      KBinsDiscretizer n_bins=200 on RaceProgress +
                       n_bins=7 on LapTime (yekenot's exact bin sizes).
  ITEM 7 (orig-aug):   WITH_ORIG_DATA=True. Per-fold concat of
                       aadigupta1601 (4/5 stratified split, weight 1.0).

Caveat: items 2-4 are NN-specific (RealMLP can't derive these).
CatBoost CAN via CTR + split-finding; expected lift smaller than for
NN. Item 7 universal (real-DGP rows in train).

Outputs:
  oof_p1_single_cb_v4_gpu_seed42.npy
  test_p1_single_cb_v4_gpu_seed42.npy
  p1_single_cb_v4_gpu_results.json
"""
...
MAX_ROUNDS = 2500
SMOKE = True               # set True for 1-fold time-probe
