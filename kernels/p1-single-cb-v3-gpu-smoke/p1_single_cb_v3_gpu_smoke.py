"""P1 single-CatBoost v3 GPU kernel — research-backed recipe.

Self-contained Kaggle script. Mirrors scripts/p1_single_cb.py +
p1_features.py exactly, inlined so no module imports are needed.

Recipe synthesis (per audit/2026-05-07-p1-cb-research-synthesis.md):
  - Bernoulli + subsample=0.8 + rsm=0.8 (irrigation proven, +13bp
    smoke vs MVS)
  - default simple_ctr / combinations_ctr / max_ctr_complexity (binary
    class needs no tuning per Garkavenko + CB docs)
  - one_hot_max_size=10 (Compound 5 + Year 4 + Stint <=5 → OHE; CTR
    capacity stays on Driver 887 + Race 26)
  - min_data_in_leaf=20, l2_leaf_reg=8.0, depth=10
  - border_count=254 (GPU max quality per CB docs)
  - 3-seed rank-bag ({42, 13, 71})
  - Year + Stint promoted to cat_features (Year importance=48.7 in M3)
  - Optional aadigupta1601 original rows w=0.5 (synthetic-DGP trick)

Datasources mounted by Kaggle (configure in kernel-metadata.json):
  /kaggle/input/playground-series-s6e5/  (competition; train.csv etc)
  /kaggle/input/f1-strategy-dataset-pit-stop-prediction/  (aadigupta_orig)
  /kaggle/input/formula-1-official-data-19502022/  (1950-2022 priors)

Outputs (written to /kaggle/working/):
  oof_p1_single_cb_v3_gpu_{seed}.npy  + test_..._strat.npy
  oof_p1_single_cb_v3_gpu_bag.npy     + test_..._bag.npy
  p1_single_cb_v3_gpu_results.json
"""
...
SEED, N_FOLDS = 42, 5
SEEDS_BAG = [42, 13, 71]
WALL_BUDGET_S = 5 * 3600  # Kaggle 9h; bag stops if >5h elapsed
WORK = Path("/kaggle/working")
WORK.mkdir(parents=True, exist_ok=True)

# === SMOKE MODE ===
# 1 fold × 1 seed × 2500-round cap on FULL data (351k ti rows). Goal:
# bound per-fold wall + confirm no further GPU param errors before
# committing 5-fold × 3-seed compute (Rule 2: smoke + 1-fold time-probe).
SMOKE = True
    ...
    for fold, (ti, vi) in enumerate(fold_list[:n_eff_folds], 1):
        ...
        for sd in SEEDS_BAG[:N_SEEDS]:
            ...
            fold_aucs_seed[sd].append(float(roc_auc_score(
                train_va[TARGET].astype(int).values, m.predict_proba(X_va)[:, 1])))
            iters_seed[sd].append(int(m.tree_count_))
